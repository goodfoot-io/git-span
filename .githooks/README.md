...
| `pre-commit.generated-trees.sh` | Reject staged `plugins-*/skills/` changes with no staged build input (`skills-src/`, registry, driver) — those trees are generated (see skills-src/README.md) | Yes; `--no-verify` for a deliberate output-only commit |
