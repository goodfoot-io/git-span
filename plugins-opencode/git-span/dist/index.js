...
function createOpencodeLogger(options = {}) {
  ...
  for (const statement of statements) {
    ...
    if (match !== null && structured.has(match[1])) {
      const keys = [...match[2].matchAll(new RegExp(`\\[(${PYTHON_STRING_SOURCE})\\]`, "g"))].map(
        (key2) => decodePythonString(key2[1])
      );
...
function planBashTouches(command, cwd, sessionId, toolUseId, logger, store) {
  const started = performance.now();
...
async function runLayeredBashTouches(command, cwd, sessionId, toolUseId, toolResponse, executors, memo, logger, store) {
  ...
  const planned = plannedSpans(record2, cwd, logger);
  const parsed = parseCommandLayered(command, { cwd, readPreState: readText });
...
// src/opencode/narrows.ts
function stringField2(args, field) {
...
function narrowBashArgs(args) {
  const command = stringField2(args, "command");
  if (command === void 0) return null;
  const workdir = stringField2(args, "workdir");
  return workdir === void 0 ? { command } : { command, workdir };
}
function narrowReadArgs(args) {
  ...
  return offset === void 0 && limit === void 0 ? { filePath } : { filePath, offset, limit };
}
...
function narrowApplyPatchText(args) {
  return stringField2(args, "patchText") ?? null;
}
function toBashResponse(outputChannel, metadata) {
  const record2 = metadata !== null && typeof metadata === "object" ? metadata : {};
  const rawText = typeof record2.output === "string" ? record2.output : typeof outputChannel === "string" ? outputChannel : "";
  const exit = record2.exit;
  const outputPath = typeof record2.outputPath === "string" && record2.outputPath.length > 0 ? record2.outputPath : void 0;
  return {
    output: rawText,
    exitStatus: Number.isInteger(exit) ? exit : void 0,
    interrupted: typeof exit !== "number",
    ...outputPath === void 0 ? {} : { rawOutputPath: outputPath }
  };
}
      ...
      if (idsUsable && input?.tool === "bash") {
        ...
        if (narrowed !== null) {
          ...
          const touch = {
            ...
            ...narrowed.offset === void 0 ? {} : { offset: narrowed.offset },
...
function postTracked(touch, cwd) {
...
function createDisposeHandler(deps) {
  const layout = deps.layout ?? DEFAULT_SESSION_LAYOUT;
  const logger = deps.logger ?? { warn: () => void 0 };
  return async () => {
...
function createOpencodeCallState() {
  ...
  return {
    ...
    takePatchPlan(sessionId, callId) {
      ...
      if (entry !== void 0) delete entry.patchPlan;
...
function createStaticPlanHandler(deps) {
  const layout = deps.layout ?? DEFAULT_SESSION_LAYOUT;
  const logger = deps.logger ?? { warn: () => void 0 };
  return async (input, output) => {
    try {
...
async function gitSpanOpencode(input = {}) {
