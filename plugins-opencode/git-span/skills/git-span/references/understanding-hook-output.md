...
- **Touch hook** (`tool.execute.after` on `bash`/`read`/`edit`/`write`, plus
  `apply_patch` on gpt models) — fires synchronously right after each shell command,
  read/edit/write, or patch completes, not after commit. It heals positional drift
  silently and, when it can't, appends a bounded `<git-span>` block to the tool
  result's output.
- **Advisor** (`tool.execute.before` on `bash`) — fires before `git commit`/`git
  push`/`git status` runs. OpenCode has no hook matchers, so both hooks filter by
  tool id themselves. For `git commit`/`git push`, the advisor holds the command once
  when the resolved changeset carries real span debt, so the report gets read; it
  never enforces, and an identical retry proceeds. A hold is a thrown error whose
  text — the full checklist, since there is no separate system-message channel —
  surfaces verbatim as the tool error. A plain `git status` is never held — it only
  reports: the same checklist is appended to the tool result after execution,
  re-reported live every call, and the check never touches the consider-once memo a
  real `git commit`/`git push` depends on.

One coverage gap is accepted for v1: OpenCode's experimental code-mode `execute`
tool is excluded from both hooks' filters — commands issued through it bypass
attribution and advisory checks alike. Prefer the regular `bash` tool when span
tracking matters.

## The touch hook: the merged `<git-span>` block

When an edit lands (or a read touches a partial range) inside a span anchor,
the touch hook appends a merged `<git-span>` block to the tool result's
output: a header line, one full span section per surfaced span (sections
separated by `---`), and a single footer after a final `---`. A healthy span
renders as:
...
never a `read`, `edit`, `write`, or patch. It resolves the actual changeset
(staged files, plus tracked-modified files when the command uses `-a`/`-am`;
for `git status`, staged plus tracked-modified — the same working-tree picture
`git status` itself prints), reruns a scoped `drift --fix`, then classifies
what's left. For `git commit`/`git push`, a hold throws an error from the
before hook, which blocks the command and surfaces the thrown text — one of
two shapes below — verbatim as the tool error. There is no separate system
message, so the checklist in the error is all the model sees:
...
Dispatch a subagent with the `task` tool to bring the coupled files back into
agreement (follow confirmed authority) — preserve anchor shape; if an address
changed, swap the old anchor for the new one with `git span replace`; update or
...
`reconcile` skill via the skill tool in the subagent. The hold will not fire
again for the same debt state. Conform a side only when confirmed authority or
a satisfied gate decides it; report ambiguity or an obsolete dependency.
```
...
Dispatch a subagent with the `task` tool to determine if this file carries implicit
...
1. Semantic drift: dispatch a subagent with the `task` tool to bring the
   coupled files back into agreement (loading the `reconcile` skill via the
   skill tool in the subagent),
...
2. Uncovered writes: dispatch a subagent with the `task` tool to determine
   whether the uncovered files carry implicit dependencies and to use
   `git span` to document them (loading the `git-span` skill via the skill
   tool in the subagent), or
...
When you need to tell them apart, set `OPENCODE_GIT_SPAN_LOG_FILE` to a path
and re-run the command; each advisor invocation appends a JSONL
`git-span advisor churn suppression` record with the candidate count, how many
were dropped by path, how many by content, how many were reported, and whether
the diff read succeeded. Without that variable set the plugin's logger has no
destination and the record is discarded, which is why a missing file cannot be
explained from the hook's normal output alone.
...
`tool.execute.before` planner and `tool.execute.after` handler share one
bounded parser for
...
ignored files, span metadata, and untracked files are dropped. The `bash`
tool's `workdir` argument (resolved by the host) is used for both planning and
post-tool attribution, so an explicit working directory inside a different
repository is handled in that repository.
...
`tool.execute.after` atomically consumes the record once, verifies the evidence,
...
reuse a plan. Under OpenCode, `session.idle` prunes the session's call-scoped
plans between turns, and `session.deleted` — plus a final `dispose()` sweep at
...
to say: a missing `git span` binary, a timed-out or failed scan, or a
...
said — never an error condition. OpenCode imposes no per-hook timeouts and
loads plugins in-process, so the plugin bounds its own `git span`
subprocesses with tightened timeouts instead of relying on a host kill
switch — a stalled scan delays the tool call briefly, then resolves to
