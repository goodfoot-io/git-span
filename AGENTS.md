...
Project documentation lives in `README.md`, `packages/git-span/docs/`, `wiki/`, and `plugins/git-span/skills/git-span/`. Keep docs focused on `git-span`, the Rust CLI in `packages/git-span`, and the lightweight VS Code extension in `packages/extension`.
