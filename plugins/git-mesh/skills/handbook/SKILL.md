...
- **A finding is `DELETED`, `MERGE_CONFLICT`, or `SUBMODULE`; or a `.mesh/` file carries git conflict markers and needs resolving (`git mesh stale --fix`)**: Read `./sections/terminal-statuses.md`
...
- **A question is about whether a git hook (`post-commit`, `post-rewrite`) is needed for meshes, or about registering the optional `.mesh/` merge driver**: Read `./sections/git-hook-setup.md`
