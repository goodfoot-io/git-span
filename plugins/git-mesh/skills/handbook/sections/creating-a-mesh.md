...
- **An anchor would point at a mesh file itself.** A mesh never anchors another mesh — the freshness link *is* the mesh's own job, so meshing one is circular and self-referential. This holds wherever mesh files live: the root is `.mesh/` by default but `--mesh-dir`/`GIT_MESH_DIR` can relocate it (e.g. `.wiki/`), so judge by whether the path is a mesh file, not by its directory name. (Wiki *pages* and other prose are fine anchors; the mesh *storage* files are not.)
