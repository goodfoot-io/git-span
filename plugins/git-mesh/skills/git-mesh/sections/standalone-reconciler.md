...
This section describes how meshes are reconciled when you are spawned as a
...
prompt wins. Load the rest of the `git-mesh` skill for judgment calls
(`./responding-to-drift.md`, `./creating-a-mesh.md`).
...
a session touched, not the commit's diff — hints, not exact spans. Extract
...
- The tip has the current `.mesh` tree. Detection at an old `sha` cannot see
  meshes landed since, so you would re-create coverage that already exists.
- `git mesh add` hashes anchors against HEAD, so anchors added at the tip are
  fresh where they land — a mesh committed at an old `sha` can be born stale
  at the tip.
...
(no `#L` ranges on `stale`; silent exit 0 means clean):

```
git mesh stale --fix -- <paths>          # auto-repair moved/whitespace drift
git mesh stale --format porcelain -- <paths>
git mesh list <paths> --oneline
```

`stale --fix` (and `history`) exit non-zero whenever findings survive, even
after repairing others — judge them by output, not exit code.

Classify into **stale anchors**, **related meshes** (an existing mesh anchors
one of the paths), and **uncovered writes** (no mesh anchors the path), then
apply `./responding-to-drift.md` and `./creating-a-mesh.md` § "Should this be
a mesh?". Most uncovered writes correctly produce nothing. A record none of
whose paths produced a finding is resolved — delete its file from your claim
directory immediately.

Re-run `git mesh stale --format porcelain -- <paths>` and confirm your paths
are clean before committing.

## Commit and land it yourself

One commit per branch group, `.mesh/` paths only:

```
git add .mesh && git commit -m "<summary>"
```

Then publish it — there is no dispatcher-side landing step:

1. If the branch moved while you worked, `git rebase <branch>`. On a `.mesh/`
   conflict: confirm the anchored sources are conflict-free, run
   `git mesh stale --fix`, continue, and re-check staleness after.
...
- Never modify, stage, or commit anything outside `.mesh/`. No `git add .`,
  no `-a`, no `--amend`, no `reset`/`clean`/`stash`, no push. The git
  allowlist in `./responding-to-drift.md` § "Batch recovery" applies.
- Do not parallelize `git mesh add` against the same mesh name — `add` has no
  locking; concurrent calls can silently lose anchors. One `add` with multiple
  anchors instead.
- Write a why for every mesh you create; rewrite an existing why only when the
