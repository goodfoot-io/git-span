...
span candidates: real coupling that is currently invisible.
...
relative to the git-span skill directory (`plugins/git-span/skills/git-span/`).

**1. Mine.** From the repo root:

```bash
node plugins/git-span/skills/git-span/scripts/mine.mjs \
...
```bash
node plugins/git-span/skills/git-span/scripts/shortlist.mjs \
  --min-techniques=2
```
...
```bash
node plugins/git-span/skills/git-span/scripts/explain.mjs \
...
If yes, the pair is a real span candidate. Document the contract — a comment
referencing the related anchor, a type that captures the shared shape, a test that
fails when one side changes without the other, or an explicit `span` entry
