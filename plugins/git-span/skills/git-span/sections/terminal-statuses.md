...
as-is" loop over `git span list --porcelain`; that erases the prompt without
doing the work the prompt exists for. Each span needs its own decision.

A user instruction like *"just re-add the anchors"* removes nothing about the
per-span confirmation below. If the shorthand sounds like a license to batch,
that is the moment to slow down and surface the conflict, not script the loop.

**Per-span process.** For each `DELETED` span:

1. **Read the why.** What relationship does this span claim?
   ```bash
   git span why <name>
   ```
...
3. **Decide per the drift rules in `./responding-to-drift.md`:**
   - Relationship still holds at a new location → re-anchor at the new extent
     (`git span remove <name> <old>` then `git span add <name> <new>`).
...
them). Then, for any `.span/` files git left marker-laden, run
`git span stale --fix` — it rewrites them into one clean version structurally
...
re-staging the span, and reporting — when an anchored source file still carries
markers or the `--why` prose diverged on both sides (no merge base); fix that
side and re-run. Finish the merge (`git add .span`, `git commit`) and run
`git span stale` again to confirm clean. See `./command-reference.md` §
...
**Known gap.** The contract above is for a `.span/` file git itself
marker-laden. A span with 2+ anchors on the same file that both drift from a
merge where the *source* conflicted but the `.span/` file did not (so this
...
**Cause.** An anchor points *inside* a submodule. git-span does not open
...
inside a submodule at anchor-creation time — is refused by `git span add`
...
closed on it (it does not corrupt the span), but the `DELETED` guidance you'll
