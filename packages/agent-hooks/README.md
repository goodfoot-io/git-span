...
## Suppressing mesh references per path

Some meshes are noise in certain parts of the tree — wiki or marketing meshes that anchor prose add little when you are reading source. A repo can hold them back with a `.mesh/.hookignore` file at its root. Each non-comment line is a gitignore-style path pattern, a single space, then a comma-separated list of mesh slug **prefixes** to suppress for anchors under matching paths:

```gitignore
# <path pattern>  <comma-separated mesh slug prefixes>
packages/agent-hooks/src  wiki,marketing
packages                  wiki
```

A mesh whose slug equals or begins with one of those prefixes (e.g. `wiki` or `wiki/onboarding`) is then never surfaced for an anchor under the matching path — neither inline by the `PreToolUse` hook while you read, nor in the `Stop` hook's stale / related sections. Suppressed meshes still count toward write coverage, so hiding one never makes a covered write look uncovered.
