...
function resolveExpected(fixture: GoldenFixture): ResponseSpan[] {
...
function sortedSpans(spans: ResponseSpan[]): ResponseSpan[] {
  return [...spans].sort(
...
function parseFixture(f: GoldenFixture, overrides: Partial<ResponseParseInput> = {}): ResponseSpan[] {
 ...
 */
function isSpanSubset(parsed: ResponseSpan[], full: ResponseSpan[]): boolean {
...
describe('parse-response (Phase 3a–3c — search layouts, unified diffs, and blame active)', () => {
  ...
  describe('distinct-span cap', () => {
    ...
    it('caps at MAX_RESPONSE_SPANS, fail-closed beyond, never capping a coalesced window', () => {
      ...
      const parse = (stdout: string): ResponseSpan[] => parseResponse({ command: 'rg -n foo', cwd: root, stdout });
