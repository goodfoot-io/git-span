/**
 * Path-scoped mesh suppression for the agent hooks.
 *
 * Some meshes are noise when browsing certain parts of the tree — wiki or
 * marketing meshes that anchor prose, surfaced inline while reading source,
 * add little. This module lets a repo declare, per path, which mesh slug
 * prefixes to hold back.
 *
 * Config lives at `<repoRoot>/.mesh/.hookignore`. Each non-comment line is a
 * gitignore-style path pattern, a single run of whitespace, then a
 * comma-separated list of mesh slug prefixes to suppress for paths the pattern
 * matches:
 *
 *   packages/agent-hooks/src wiki,marketing
 *
 * A mesh whose slug begins with `wiki` or `marketing` (the slug equals the
 ...
 * malformed line, yields no rule rather than hiding meshes the author did not
 * ask to hide.
 */
...
export interface IgnoreRule {
  ...
  pattern: string;
  /** Mesh slug prefixes suppressed for paths this rule matches. */
  prefixes: string[];
  /** True when `repoRelPath` (POSIX, repo-relative) is governed by this rule. */
  matches: (repoRelPath: string) => boolean;
}

const HOOK_IGNORE_REL = nodePath.join('.mesh', '.hookignore');
 ...
 * yields an empty rule set, so meshes surface as normal when no config exists.
 */
...
/**
 * True when a mesh `slug` should be suppressed for an anchor at `repoRelPath`:
 * some rule matches the path and lists a prefix the slug carries.
 */
export function isMeshSuppressed(rules: IgnoreRule[], repoRelPath: string, slug: string): boolean {
