...
/**
 * A response-derived read span: always an exact line range in one file.
 * Distinct from the command parser's {@link ResolvedSpan} — response spans
 * carry no `operation`/`simpleCommandIndex` (they are not tied to a simple
 * command; the adapters run each as a bare read touch), and their range is
 * always present, because every response decode locates its read window in
 * the output.
 */
 ...
 */
function capSpans(spans: ResponseSpan[]): ResponseSpan[] {
 ...
 * ranges.
 */
function spansFor(perFile: Map<string, Set<number>>, baseDir: string, roots: string[]): ResponseSpan[] {
  const spans: ResponseSpan[] = [];
 ...
 * matcher.
 */
export function parseResponse(input: ResponseParseInput): ResponseSpan[] {
