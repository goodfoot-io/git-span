...
//! cost of `git mesh stale` on large meshed repos.
//!
...
//!
//! This test commits K renamed anchored files, runs `git mesh stale`, and
...
fn relocation_scan_reads_are_linear_in_candidate_paths() -> Result<()> {
    ...
    // Anchor a line range in each file in its own mesh.
    for i in 0..K {
        let mesh = format!("m{i:02}");
        let pinned = format!("src/file{i:02}.txt#L1-L3");
        repo.run_mesh(["add", &mesh, &pinned])?;
        repo.run_mesh(["why", &mesh, "-m", "seed"])?;
    }
    repo.run_git(["add", ".mesh"])?;
    repo.run_git(["commit", "-m", "seed meshes"])?;
