...
//!
//! - R2-C1: `git mesh add` is fail-closed on unsafe/invalid anchor
...
fn add_rejects(repo: &TestRepo, path: &str) -> Result<()> {
    let out = repo.run_mesh(["add", "bad/x", path])?;
    anyhow::ensure!(
        !out.status.success(),
        "`git mesh add bad/x {path}` must exit non-zero (fail-closed); \
         stdout={} stderr={}",
...
fn add_accepts_legitimate_anchor_path() -> Result<()> {
    ...
    let out = repo.run_mesh(["add", "ok/y", "f.txt"])?;
    anyhow::ensure!(
        out.status.success(),
...
fn add_accepts_submodule_gitlink_whole_file_anchor() -> Result<()> {
    ...
    let out = repo.run_mesh(["add", "demo/s", "sub"])?;
    anyhow::ensure!(
        out.status.success(),
...
/// Seed a mesh with a line anchor and a whole-file anchor, committed.
fn seed_two_anchors(repo: &TestRepo) -> Result<()> {
    repo.write_file("src/foo.txt", "l1\nl2\nl3\nl4\nl5\n")?;
    repo.write_file("src/bar.txt", "bar\n")?;
    repo.commit_all("init")?;
    repo.mesh_stdout(["add", "demo/flow", "src/foo.txt#L2-L4"])?;
    repo.mesh_stdout(["add", "demo/flow", "src/bar.txt"])?;
    repo.mesh_stdout(["why", "demo/flow", "-m", "seed"])?;
    repo.commit_all("seed mesh")?;
    Ok(())
}

fn stale(repo: &TestRepo) -> Result<String> {
    repo.mesh_stdout(["stale", "--no-exit-code"])
}
...
fn delete_prunes_empty_parent_dirs() -> Result<()> {
    ...
    repo.write_file("f.txt", "hi\n")?;
    repo.commit_all("init")?;
    repo.mesh_stdout(["add", "bulk/foo", "f.txt"])?;
    assert!(repo.path().join(".mesh/bulk").is_dir());

    repo.mesh_stdout(["delete", "bulk/foo"])?;
    assert!(
        !repo.path().join(".mesh/bulk").exists(),
        "the now-empty `.mesh/bulk/` parent dir must be pruned"
    );
    assert!(
        repo.path().join(".mesh").is_dir(),
        "the mesh root itself must be preserved"
    );
    Ok(())
}
...
fn delete_keeps_non_empty_parent_with_sibling() -> Result<()> {
    ...
    repo.write_file("f.txt", "hi\n")?;
    repo.commit_all("init")?;
    repo.mesh_stdout(["add", "grp/a", "f.txt"])?;
    repo.mesh_stdout(["add", "grp/b", "f.txt"])?;
    repo.mesh_stdout(["delete", "grp/a"])?;
    assert!(
        repo.path().join(".mesh/grp/b").is_file(),
        "a sibling mesh under the same parent must be preserved"
    );
    Ok(())
}
...
fn move_prunes_empty_old_parent_dirs() -> Result<()> {
    ...
    repo.write_file("f.txt", "hi\n")?;
    repo.commit_all("init")?;
    repo.mesh_stdout(["add", "bulk/bar", "f.txt"])?;
    repo.mesh_stdout(["move", "bulk/bar", "top"])?;
    assert!(
        !repo.path().join(".mesh/bulk").exists(),
        "the now-empty old `.mesh/bulk/` parent dir must be pruned"
    );
    assert!(repo.path().join(".mesh/top").is_file());
    Ok(())
}
...
fn add_success_message_has_no_ref_era_framing() -> Result<()> {
    ...
    repo.write_file("f.txt", "hi\n")?;
    repo.commit_all("init")?;
    let out = repo.mesh_stdout(["add", "m/x", "f.txt"])?;
    assert!(
    ...
    assert!(
        out.contains("Added 1 anchor to mesh `m/x`."),
...
fn malformed_show_surfaces_only_parse_error() -> Result<()> {
    ...
    std::fs::write(repo.path().join(".mesh/broken"), "garbage not valid\n")?;

    let out = repo.run_mesh(["show", "broken"])?;
    anyhow::ensure!(
        !out.status.success(),
        "showing a malformed mesh must exit non-zero"
    );
    let combined = format!(
        "{}{}",
        String::from_utf8_lossy(&out.stdout),
    ...
    assert!(
        combined.contains("invalid mesh file"),
        "a malformed mesh's `show` must surface the parse error; got=\n{combined}"
    );
    assert!(
        !combined.contains("no mesh named"),
        "a mesh that exists but fails to parse must not be reported as \
         'no mesh named'; got=\n{combined}"
    );
    Ok(())
}
...
fn moved_whole_file_stat_shows_moved_not_counts() -> Result<()> {
    ...
    repo.commit_all("init")?;
    repo.mesh_stdout(["add", "demo/m", "src/w.txt"])?;
    repo.mesh_stdout(["why", "demo/m", "-m", "seed"])?;
    repo.commit_all("seed mesh")?;
    std::fs::create_dir_all(repo.path().join("src"))?;
    repo.run_git(["mv", "src/w.txt", "src/moved.txt"])?;
    repo.run_git(["commit", "-m", "git mv"])?;

    let s = repo.mesh_stdout(["stale", "--stat", "--no-exit-code"])?;
    assert!(
        s.contains("moved"),
