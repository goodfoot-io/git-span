...
//! (`stale_meshes_inner`) for meshes NOT in the dirty set.
//!
...
//! is skipped: the dirty path rendered `apply_overlay(&baseline.meshes,
//! &overlay)`, where `baseline.meshes` is the row-level COMMITTED-ONLY baseline
//! and only meshes touched by a dirty path receive the effective overlay. Every
//! UNAFFECTED mesh was rendered straight from the `committed_only` baseline,
...
//!      working tree".
//!
//! Additionally the committed_only baseline can order a mesh's anchors
...
//!
//! The fix routes the dirty path's non-affected committed meshes through the
...
/// Multi-mesh corpus with a MOVED finding and a CHANGED finding, made dirty by
/// touching an UNRELATED tracked file. The moved/changed meshes are NOT in the
...
/// divergence.
fn seed_moved_multi_mesh_dirty(repo: &TestRepo) -> Result<()> {
    repo.write_file("a.txt", "AAA\nBBB\nMOVED_X\nMOVED_Y\nCCC\n")?;
    repo.write_file("z.txt", "ZZZ\nQQQ\nWWW\n")?;
    repo.write_file("unrelated.txt", "u1\nu2\nu3\n")?;
    repo.commit_all("seed")?;

    repo.run_mesh(["add", "aaa", "a.txt#L3-L4"])?;
    repo.run_mesh(["why", "aaa", "-m", "moved content tracker"])?;
    repo.run_mesh(["add", "zzz", "z.txt#L1-L2"])?;
    repo.run_mesh(["why", "zzz", "-m", "changed content tracker"])?;
    ...
    // DIRTY the tree by editing an UNRELATED tracked file (no mesh anchors it),
...
#[test]
fn dirty_tree_moved_multi_mesh_matches_cache_off_all_formats() -> Result<()> {
    let repo = TestRepo::new()?;
    seed_moved_multi_mesh_dirty(&repo)?;
...
/// The interior-anchor mesh is NOT in the dirty set, so it renders from the
...
fn seed_interior_anchor_dirty(repo: &TestRepo) -> Result<()> {
    ...
    repo.run_mesh(["add", "normal", "a.txt#L1-L3"])?;
    repo.run_mesh(["why", "normal", "-m", "normal mesh"])?;
    repo.run_git(["add", ".mesh"])?;
    repo.run_git(["commit", "-m", "mesh normal"])?;

    // Interior-anchor mesh whose anchor path is under `.mesh/`.
    ...
    // Drift the normal mesh's anchored region via a COMMIT (clean tree), so the
    // interior anchor over `.mesh/normal` is itself drifted at HEAD.
    repo.write_file("a.txt", "A1\nA2\nA3\na4\na5\n")?;
    repo.commit_all("drift a")?;
    repo.write_commit_graph()?;

    // DIRTY an UNRELATED tracked file: the poison (interior) mesh is unaffected
