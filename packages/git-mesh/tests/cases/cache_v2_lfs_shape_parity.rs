...
/// Seed a mesh anchoring a file under `sub/`, whose LFS configuration lives in
...
fn seed_subdir_lfs_corpus(repo: &TestRepo) -> Result<()> {
    ...
    repo.run_mesh(["add", "m", "sub/big.txt#L1-L3"])?;
    repo.run_mesh(["why", "m", "-m", "anchors an LFS-tracked file"])?;
