//! Reproduction test: `git mesh stale` must emit
...
//! ## Test design
//!
//! 1. Create a git repo with a file and a mesh anchored to it.
//! 2. Commit the file and the mesh, write the commit-graph.
...
//! 5. Resolve the mesh with `EngineOptions::committed_only()` (HEAD-layer
...
use crate::support;
use anyhow::Result;
use git_mesh::types::{AnchorStatus, EngineOptions, UnavailableReason};
...
fn promisor_missing_blob_misclassified_as_deleted() -> Result<()> {
    ...
    // Set up a mesh pointing to file1.txt.
    repo.mesh_stdout(["add", "test-mesh", "file1.txt#L1-L5"])?;
    repo.mesh_stdout(["why", "test-mesh", "-m", "promisor reproduction test"])?;
    repo.run_git(["add", ".mesh"])?;
    repo.run_git(["commit", "-m", "mesh commit"])?;
    repo.write_commit_graph()?;

    // Sanity: the mesh file and anchored file exist in the worktree.
    assert!(
        repo.path().join(".mesh/test-mesh").exists(),
        ".mesh/test-mesh must exist in the worktree"
    );
    assert!(
