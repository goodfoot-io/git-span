...
/// Read the full text of a mesh file from the worktree.
fn read_mesh(repo: &TestRepo, name: &str) -> Result<String> {
    let path = repo.path().join(".mesh").join(name);
    Ok(std::fs::read_to_string(path)?)
}

/// Create a mesh with one line-range anchor and commit it.
...
fn fuzzy_above_threshold_moved() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L50", "above-threshold fuzzy")?;
    repo.write_commit_graph()?;
...
fn fuzzy_below_threshold_not_moved() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L15", "below-threshold fuzzy")?;
    repo.write_commit_graph()?;
    ...
    // --- JSON assertions ---
    let json_out = repo.run_mesh(["stale", "--format", "json", "--no-exit-code"])?;
...
fn fuzzy_no_candidates_deleted() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L10", "no-candidates fuzzy")?;
    repo.write_commit_graph()?;
...
fn fuzzy_exact_match_moved_no_fuzzy() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L10", "exact-match move")?;
    repo.write_commit_graph()?;
...
fn fix_respects_default_fuzzy_threshold() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L25", "fix threshold guard")?;
    repo.write_commit_graph()?;

    repo.run_git(["rm", "file1.txt"])?;

    // Read mesh content BEFORE --fix.
    let before = read_mesh(&repo, "m")?;

    // Default threshold: 0.852 < 0.95 -> not re-anchored.
    let fix_out = repo.run_mesh(["stale", "--fix"])?;

    let after = read_mesh(&repo, "m")?;
    assert_eq!(
        before, after,
        "mesh must be unchanged by --fix with default threshold"
    );
...
fn fix_with_lowered_fuzzy_threshold() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L25", "fix lowered threshold")?;
    repo.write_commit_graph()?;
    ...
    let fix_out =
        repo.run_mesh(["stale", "--fix", "--fuzzy-threshold", "0.70"])?;
    let _fix_stdout = String::from_utf8_lossy(&fix_out.stdout);

    let after = read_mesh(&repo, "m")?;
    assert!(
        after.contains("file2.txt"),
        "mesh must be re-anchored to file2.txt with --fuzzy-threshold 0.70; mesh:\n{after}"
    );
    assert!(
        !after.contains("file1.txt"),
        "old anchor path must be removed from mesh; mesh:\n{after}"
    );
...
fn fuzzy_json_output() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L50", "json fuzzy")?;
    repo.write_commit_graph()?;
...
fn fuzzy_porcelain_comment() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L50", "porcelain fuzzy")?;
    repo.write_commit_graph()?;
...
fn fuzzy_porcelain_below_threshold_comment() -> Result<()> {
    ...
    seed_mesh(&repo, "m", "file1.txt#L1-L15", "porcelain below threshold")?;
    repo.write_commit_graph()?;
