//! CLI: `git mesh merge-driver` -- integration tests for the Git merge driver.
//!
...
/// Helper: write mesh content to a temp file and return its path.
fn write_mesh_file(dir: &Path, name: &str, content: &str) -> std::path::PathBuf {
    let p = dir.join(name);
...
fn merge_driver_resolves_clean_three_way() {
    ...
    let base = write_mesh_file(dir.path(), "base", "a.txt#L1-L5 rk64:111\n\nwhy\n");
    let ours = write_mesh_file(
        dir.path(),
        "ours",
        "a.txt#L1-L5 rk64:111\nb.txt#L1-L3 rk64:222\n\nwhy\n",
    );
    let theirs = write_mesh_file(
        dir.path(),
        "theirs",
...
fn merge_driver_exits_non_zero_on_divergence() {
    ...
    let base = write_mesh_file(dir.path(), "base", "x.txt#L1-L5 rk64:111\n\n");
    let ours = write_mesh_file(
        dir.path(),
        "ours",
