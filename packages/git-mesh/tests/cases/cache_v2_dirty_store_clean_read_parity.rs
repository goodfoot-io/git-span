...
//! (`source_tree_key`/`mesh_tree_key`), which uncommitted worktree edits to
//! a committed `.mesh/<name>` file do NOT change. The entry's content used
//! to be derived from the worktree-effective corpus
//! (`load_all_meshes_in`). So a cold build performed while a committed mesh
...
//! from the `GIT_MESH_CACHE_V2=0` ground truth.
//!
//! The fix builds the whole-result content from COMMITTED mesh reads so the
...
/// Seed a mesh `m` with three anchors, drift one anchor's file and commit
...
fn cache_matches_cache_off_after_dirty_store_then_clean_read() -> Result<()> {
    ...
    // Worktree-only edit to the committed `.mesh/m`: add an anchor on
    // d.txt. The committed key (HEAD mesh tree) is unchanged; the tree is
    // now dirty.
    repo.run_mesh(["add", "m", "d.txt#L1-L2"])?;
    assert!(
        repo.path().join(".mesh/m").exists(),
        "mesh file missing after add"
    );
    ...
    let _dirty = repo.run_mesh(["stale", "--no-exit-code"])?;

    // Revert the worktree mesh to the committed version WITHOUT committing:
    // tree is clean again, committed key unchanged, no cache rebuild.
    repo.run_git(["checkout", "--", ".mesh"])?;
