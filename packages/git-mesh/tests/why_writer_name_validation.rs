//! `why -m`/`-F` bypass `validate_mesh_name`, allowing path-traversal
//! names like `../escape` to write outside `.mesh/`.
//!
//! Every other handler (`add`, `remove`, `why` reader, `why --edit`)
//! calls `validate_mesh_name` first.  The `-m` and `-F` writer paths
//! skip this check.
...
fn rejects_path_traversal_name_in_why_m() -> Result<()> {
    ...
    let out = repo.run_mesh(["why", "../escape", "-m", "test body"])?;

    // The -m path does not call validate_mesh_name, so it currently
    ...
    // No file should be written outside .mesh/.
    let escaped = repo.dir.path().join("escape");
    assert!(
        !escaped.exists(),
        "BUG: why -m wrote file outside .mesh/ at {:?}",
        escaped
    );
...
fn rejects_path_traversal_name_in_why_f() -> Result<()> {
    ...
    let out = repo.run_mesh(["why", "../escape", "-F", "body.txt"])?;
    ...
    assert!(
        !escaped.exists(),
        "BUG: why -F wrote file outside .mesh/ at {:?}",
        escaped
    );
