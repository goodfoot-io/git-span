    ...
    fn render_with_mixed_steps() {
        let err = CliError {
            ...
            next_steps: vec![
                ...
                NextStep::Bash(
                    "git add web/checkout.tsx\ngit mesh add billing/checkout web/checkout.tsx"
                        .into(),
                ),
            ],
        };
        let expected = "\
git mesh add: anchor path does not exist.
...
```";
    ...
    fn from_lib_error_wraps_correctly() {
        ...
        let err = from_lib_error(
            ...
            summary,
            &lib_err,
            vec![NextStep::Bash("git mesh list".into())],
        );

        assert_eq!(err.subcommand, "delete");
        assert_eq!(err.summary, "no mesh named `checkout`.");
        assert_eq!(err.what_happened, "`.mesh/checkout` does not exist");
