    ...
    let mesh = {
        ...
        let conflict_err = |name: &str| CliError {
            ...
            next_steps: vec![
                ...
                NextStep::Prose("Resolve the merge conflict in the mesh file, then retry.".into()),
            ],
        };
        if args.at.is_none() {
            crate::mesh::read::read_mesh_in(repo, &args.name, mesh_root).map_err(|e| {
                if matches!(e, crate::Error::MeshConflict(_)) {
                    ...
                    CliError {
                        ...
                        next_steps: vec![NextStep::Bash(format!("git mesh doctor {}", args.name))],
                    }
                }
            })?
        } else {
        ...
        if !in_scope.is_empty() {
            ...
            return Err(CliError {
                ...
                next_steps: vec![
                    ...
                    NextStep::Prose(
                        "Resolve the merge conflict in the mesh file(s), then retry.".into(),
                    ),
                ],
            }
            .into());
            ...
            Err(err) => {
                return Err(CliError {
                    ...
                    next_steps: vec![
                        ...
                        NextStep::Bash("git mesh list --search \"pattern\"".into()),
                    ],
                }
                .into());
            }
