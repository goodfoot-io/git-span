...
//!
//! Usage:
//!   bench-corpus-gen <output-dir> <seed> <mesh-count> [--with-commit-graph]
//!
//! * `output-dir`  — directory that will be created and become the git repo.
//! * `seed`        — u64 seed; same seed + same mesh-count → same commit SHAs.
//! * `mesh-count`  — number of source files and mesh files to create.
...
fn main() -> anyhow::Result<()> {
    ...
    if args.len() < 4 {
        eprintln!(
            "Usage: bench-corpus-gen <output-dir> <seed> <mesh-count> [--with-commit-graph]"
        );
        std::process::exit(1);
    }
    ...
    let mesh_count: usize = args[3]
        .parse()
        .map_err(|e| anyhow::anyhow!("invalid mesh-count: {e}"))?;
    let with_commit_graph = args.iter().any(|a| a == "--with-commit-graph");

    std::fs::create_dir_all(&output_dir)?;

    git_mesh::bench_corpus::generate(&output_dir, seed, mesh_count, with_commit_graph)?;
