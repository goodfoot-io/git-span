...
- `.gitignore` — ignores generated runtime artifacts (log files) so they don't get tracked. Still written by `ensure_span_dir`; the current agent runtime (see [Agent integration](/docs/agent-integration)) is just the in-session touch hook and gate, neither of which writes files under `.span/` that need ignoring, but the file is harmless to keep around.
