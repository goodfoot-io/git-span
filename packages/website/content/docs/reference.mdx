...
- `.gitignore` — ignores generated runtime artifacts (log files and dispatch scripts) from an earlier commit-triggered reconciliation pipeline that has since been retired in favor of the in-session touch hook and gate (see [Hooks & CI integration](/docs/hooks-and-ci)). Still written by `ensure_span_dir` for forward compatibility; nothing in the current agent runtime generates the artifacts it ignores.
- `.hookignore` — path-scoped suppression rules for the touch hook's context injection.
- `.gateignore` — user-owned (nothing auto-creates it); each non-comment line is a gitignore-style path pattern, same grammar as `.hookignore`, excluding matching paths from the commit gate's uncovered-writes check.
- `.manual-run` — a presence-only marker, created automatically on the first `add`/`why`. Its original purpose — suspending an auto-spawned background reconciler agent in favor of a runnable script — belonged to the same retired pipeline as `.gitignore` above and currently has no consumer.
