...
- `.gitignore` — ignores generated runtime artifacts (log files) so they don't get tracked. Still written by `ensure_span_dir`; the current agent runtime (see [Hooks & CI integration](/docs/hooks-and-ci)) is just the in-session touch hook and gate, neither of which writes files under `.span/` that need ignoring, but the file is harmless to keep around.
...
- `.manual-run` — a presence-only marker, created automatically on the first `add`/`why`. It's a legacy artifact with no consumer in the current touch/gate hook model — nothing reads it — and is safe to ignore or gitignore.
