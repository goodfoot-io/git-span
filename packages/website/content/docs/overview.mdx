...
2. **Explain** it: `git span why <name> "..."` attaches a sentence saying what the anchors collectively do. The why is the payload; a span without one is just a list of locations.
...
git-span is designed to be operated by coding agents. It ships as plugins for Claude Code and OpenAI Codex whose in-session hooks keep spans honest as an agent works: a touch hook heals positional drift right after each edit, and a commit advisor holds commits that carry real span debt so the report gets read. Every capability behind those hooks is still a plain, scriptable CLI — the same commands work by hand, and the same `git span stale` exit code is what CI checks at merge time.
