...
<Callout type="info">
`--format json` always emits a document — a clean scan carries `"clean": true, "findings": []` (schema_version 3) rather than empty stdout, so a parser can rely on an envelope on every run and read `clean` for the verdict. `--format porcelain` still prints nothing on a clean scan (no header row); empty stdout plus exit 0 is the clean signal there. The human format (the default) prints a summary line on a clean scan.
</Callout>
