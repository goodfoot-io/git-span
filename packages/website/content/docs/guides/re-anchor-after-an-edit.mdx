...
If the code moved to a different range, add the new address and drop the drifted one:
...
`add` stages the new anchor into the span; `remove` drops the drifted one. Order doesn't matter — both edit the span file under the span root.
