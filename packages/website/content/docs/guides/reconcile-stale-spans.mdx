...
After running `--fix`, re-run `git span stale` and treat anything still listed as drift as needing manual re-anchoring. See [Re-anchor after an edit](/docs/guides/re-anchor-after-an-edit) for the `git span add` / `git span remove` steps.
...
- **The anchors now disagree.** Conform the non-authoritative side before touching the span: a doc that lags a deliberately committed code change is rewritten to describe current reality, then the span is re-anchored onto it. When the fix would change code, when the doc may be the intended contract rather than a description, or when no deliberate commit explains the drift, stop and surface the decision instead — re-anchoring over a live disagreement records the contradiction as the new baseline.
...
`why` shows the span's current rationale; `history` walks the span's commit-by-commit timeline, including what each anchor's content looked like when it was added or last modified. Use both to find the commit that caused the drift — a deliberate, committed change makes that side authoritative — and to judge whether a meaning-altering edit broke the coupling or just relocated it.
