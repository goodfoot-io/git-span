---
title: Reconcile drifted spans
description: Find drifted anchors with git span drift, auto-resolve what's mechanically safe, and manually re-anchor the rest.
---
...
After running `--fix`, re-run `git span drift` and treat anything still listed as drift as needing manual re-anchoring. See [Re-anchor after an edit](/docs/guides/re-anchor-after-an-edit) for the `git span add` / `git span remove` steps.
