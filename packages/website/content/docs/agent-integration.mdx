...
The git-span plugin is how an agent operates git-span without being told the command syntax on every turn. Once installed (see [Getting started](/docs/getting-started)), it provides these capabilities:
...
- **An expert subagent where the host supports one** — Claude Code, Codex, and OpenCode ship a dedicated agent for judgment-heavy span work. Antigravity ships the skills and hooks without a separate agent artifact.
...
The bundles also include lifecycle and planning hooks that attribute supported writes, clean up per-session state, and check CLI/plugin compatibility. Event names, delivery channels, and tool coverage differ by host; the touch hook and advisor are the two user-facing workflows.

## Touch hook + advisor

- **Touch hook** — observes supported file-affecting tool calls. Claude Code covers `Read`, `Edit`, `Write`, and `Bash`; Codex covers `apply_patch` and its shell/exec tool family; OpenCode covers `bash`, `read`, `edit`, `write`, and `apply_patch`; Antigravity covers `run_command`. It re-anchors pure positional drift against the file's real post-edit content, silently. If genuine semantic drift survives that heal, it injects a bounded context block naming the span, its status, and what to do. Each `(span, status)` pair surfaces at most once per session.
...
- **Coverage.** The hooks cover the `bash`, `read`, `edit`, and `write` tools, plus `apply_patch` on gpt models (where it replaces `edit`/`write`). OpenCode's experimental code-mode `execute` tool is excluded from both hooks, so commands issued through it bypass attribution and advisory checks.
...
Antigravity installs a plugin directory with three skills and seven hook entry points. There is no marketplace or trust-review step. The host treats an empty `PreToolUse` response as a denial, so every non-hold path returns an explicit allow; a real hold denies `git commit` or `git push` once with the checklist as its reason, and an identical retry proceeds.

Only `run_command` has a pinned tool contract. Shell activity receives the planning, attribution, drift-healing, and advisor workflows described above. Dedicated file-edit tools do not: their names and argument shapes are not stable enough for the plugin to guess. Their changes remain visible to the advisor because it resolves the actual git changeset, and `git span drift` in CI remains the enforcement backstop.

Antigravity's `PostToolUse` event carries a step index rather than the original call or response. The plugin joins it to disk-backed pre-call state, queues rendered context, and delivers that context after the invocation as an ephemeral message. Successful commands provide no exit-code or response-derived read evidence; failed commands are handled conservatively. This changes delivery timing and attribution detail, not the span resolver's meaning.

## The enforcement backstop

The touch hook and advisor only run inside a hooked agent session. A human-authored commit, a session without active hooks, a tool outside a host's coverage, or a failed host-level tool call can still land span debt. [CI & git integration](/docs/ci) covers `git span drift` as the backstop that catches what the in-session hooks miss.
