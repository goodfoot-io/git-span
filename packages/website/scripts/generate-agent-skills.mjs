 ...
 * only input. `scripts/` → three levels up. Exported so the umbrella artifact
 * gate can regenerate from the same root. */
export const defaultSkillsRoot = fileURLToPath(new URL('../../../plugins-claude/git-span/skills', import.meta.url));
...
function* walkFiles(root, dir = root) {
  // Sorted at every level: readdir order varies by filesystem, and the
  // emitted artifact is byte-gated — an order flip would read as drift on a
  // different machine even though nothing changed.
 ...
 * lint or pre-commit pass leaves its bytes untouched. `outPath` is
 * parameterized so the umbrella artifact gate can emit to a temporary
 * directory and byte-compare — generate and check share this one code path.
 */
export function emit(publication, outPath = path.join(packageRoot, 'app/lib/agent-skills.generated.ts')) {
  const json = JSON.stringify(publication, null, 2);
  const source = `/**
 * GENERATED by packages/website/scripts/generate-agent-skills.mjs — do not
 * edit by hand; regenerate with
 * \`yarn workspace @goodfoot/git-span-website generate:artifacts\`.
 *
 * The agent-skills publication emitted from the normative Claude plugin skill
 * tree. The contract suite fails when this file differs from a fresh
 * generator run.
 */
