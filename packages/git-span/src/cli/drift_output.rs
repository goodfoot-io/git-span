...
pub fn run_drift(repo: &gix::Repository, args: DriftArgs, span_root: &str) -> Result<i32> {
    ...
    // and read back in the `fix.*` emit block at the end of `run_drift`.
    ...
    if args.fix && !matches!(args.format, DriftFormat::Human) {
        return Err(CliError {
            subcommand: "drift",
            ...
            what_happened: "Non-human formats stream drifted findings only and have no \
                            post-rewrite view."
                .into(),
            next_steps: vec![NextStep::Bash("git span drift --fix".into())],
        }
        .into());
    }
    ...
    // can short-circuit once HEAD says "drift".
    let needs_all_layers = matches!(args.format, DriftFormat::Human);
    ...
    if args.perf_trace.is_some() && !args.paths.is_empty() {
        return Err(CliError {
            ...
            what_happened: "--perf-trace captures per-anchor timing across all spans and cannot \
                ...
                .into(),
            next_steps: vec![NextStep::Bash("git span drift --perf-trace <path>".into())],
        }
        .into());
    }

    // For a scoped query (positional args), the "0 drift" summary reports the
    ...
    // None means "no scope restriction" (bare `git span drift`).
    ...
    // backfilled anchor set + anchor totals, allowing run_drift to skip its
    ...
    let mut spans = if args.paths.is_empty() {
        ...
        // included — workspace scans answer "what's drift?" only.
        let _perf = crate::perf::span("drift.resolve-all-spans");
        if let Some(trace_path) = &args.perf_trace {
            ...
            let (resolved, trace_rows) = drift_spans_with_trace(repo, span_root, options)
            ...
            let (resolved, layers, wr) =
                drift_spans_retaining_source_layers(repo, span_root, options)
                    .map_err(curate_resolver_failure)?;
            pre_fix_source_layers = layers;
            whole_result = wr;
            resolved
        } else {
            let (resolved, _, wr) = drift_spans_retaining_source_layers(repo, span_root, options)
                .map_err(curate_resolver_failure)?;
            whole_result = wr;
            resolved
        }
        ...
        let _perf = crate::perf::span("drift.resolve-args");
        ...
        for arg in &args.paths {
            ...
            // span tracks it, so there is nothing for drift to scan and that
        ...
        let resolved = {
            let _perf = crate::perf::span("drift.resolve-named-spans");
            if args.fix {
        ...
        // "0 drift" summary instead (see the no-drift block below).
        // Conflict-injected spans are added after this block and carry a
        // `MergeConflict` anchor, so they are unaffected.
        // `m.anchors` may hold only the drifted subset (the cache_v2 resolver
        ...
        // Two drift predicates coexist in this file, deliberately.
        ...
        // drift-findings count, cluster membership) uses the narrower
        // [`anchor_status_is_drift`], the same boundary `git span
    ...
    let fix_result: Option<FixResult> = if args.fix {
        ...
        //     its interior anchor against drift (pre-fix) span-file content.
        ...
        // retains `SourceLayers` on the cold (`drift_spans_inner`) path; on a
        ...
        if pre_fix_source_layers.is_none() && !has_interior_anchor {
            let _perf = crate::perf::span("drift.fix-build-source-layers");
        ...
        let fr = super::drift_fix::apply_fix(repo, &fix_input, span_root, options.fuzzy_threshold)?;
        ...
        if has_interior_anchor {
            ...
            if args.paths.is_empty() {
                spans = drift_spans(repo, span_root, options)?;
            } else {
    ...
    if matches!(args.format, DriftFormat::Human) && args.paths.is_empty() && !use_whole_result {
        ...
        // the actionable [`anchor_status_is_drift`] boundary the exit
    ...
    let findings: Vec<Finding> = {
        let _perf = crate::perf::span("drift.build-findings");
        spans
            .iter()
            .flat_map(|m| {
                m.anchors
    ...
    let drift_findings: usize = findings
        .iter()
        .filter(|f| {
        ...
        .count();
    // Interior-anchor surfacing (CARD AC4: surfaced at drift/validate time).
    ...
    if !interior_violations.is_empty() {
        eprintln!();
        eprintln!(
            "# span drift: {} interior-anchor violation(s)",
    ...
    let clusters: Vec<crate::cli::drift_cluster::DriftCluster> = if args.cluster && drift_count > 0
    {
        let _perf = crate::perf::span("drift.cluster-corpus-load");
        let (cluster_corpus, _conflicted) = crate::span::read::load_all_spans_in(repo, span_root)?;
        let drift_span_names: std::collections::BTreeSet<String> = findings
            .iter()
            .filter(|f| {
                if !anchor_status_is_drift(&f.status) {
                    return false;
        ...
        let full_anchor_paths: HashMap<String, std::collections::BTreeSet<String>> = cluster_corpus
            ...
            .collect();
        crate::cli::drift_cluster::cluster_drift_spans(&drift_span_names, &full_anchor_paths)
    } else {
        Vec::new()
    };

    // When the index changes between the start and end of a drift scan, every
    ...
    // retry. Exit code 2 is distinct from both 0 (clean) and 1 (drift).
    let index_changed = pre_fix_source_layers
        .as_ref()
        .is_some_and(|l| l.index_changed);
    if index_changed {
        eprintln!(
            "git span drift: index changed during scan; result is indeterminate — re-run the command"
        );
        return Ok(2);
    }
    ...
    match args.format {
        DriftFormat::Human => {
            let _perf = crate::perf::span("drift.render-human");
        ...
        DriftFormat::Porcelain => {
            let _perf = crate::perf::span("drift.render-porcelain");
...
/// Re-resolve only the spans that `apply_fix` rewrote and splice the results
/// back into the pre-fix set, for the **named-scope** arm (`git span drift
...
/// spans fed to `apply_fix`, and fresh sibling anchors of a drifted span on
...
fn render_human(
    ...
    for m in spans.iter() {
        ...
        if options.stat {
            // Only anchors that are actually drift belong in `--stat`
            // output. Listing fresh `+0 -0` rows (or saying "All anchors
            // … are drifted" when only some are) misleads triage. Display
            // predicate again (non-Fresh, informational rows included), to
            // match the bullets `--stat` annotates.
            let drift_findings: Vec<&Finding> = span_findings
                .iter()
                .copied()
                .filter(|f| f.status != AnchorStatus::Fresh)
                .collect();
            ...
            // `m.anchors` holds only the drifted subset, so
            // `span_findings.len()` would equal `span_drift` and the
            // heading would falsely read "All anchors … are drifted" even
            ...
            if span_drift > 0 && span_drift == span_total {
                println!("All anchors in {} are drifted:", m.name);
            } else if span_drift > 0 {
                println!(
                    "{} of {} anchors in {} are drifted:",
                    span_drift, span_total, m.name
                );
            }
...
/// on its commit read path) while `drift`'s renderer decodes them lossily.
///
...
/// `drift`'s view of the shared line-range policy
...
fn render_porcelain(
    ...
    // rows. `spans`/`shared_files` are already sorted (see `DriftCluster`).
    for c in clusters {
        println!(
            "# cluster {} shared:{}",
            c.spans
