//! Drifted-span clustering — `git span drift --cluster`.
//!
//! Connected components over the run's drifted spans, with an edge between
//! two spans whenever they share an anchored file (drifted or healthy anchor).
//! See `plans/bounded-rename-chain.md` ("Clustering design").

/// One connected component of drifted spans that share at least one anchored
/// file, transitively.
...
/// Partition `drift_span_names` into connected components by shared anchored
/// file. `full_anchor_paths` maps each span name to the full set of paths it
/// anchors (drifted or healthy) — not just its currently-drifted anchors — per
...
///
/// Only `drift_span_names` (a `BTreeSet`, so already sorted) is iterated;
...
/// iteration order.
pub fn cluster_drift_spans(
    drift_span_names: &std::collections::BTreeSet<String>,
    full_anchor_paths: &std::collections::HashMap<String, std::collections::BTreeSet<String>>,
) -> Vec<DriftCluster> {
    use std::collections::{BTreeMap, BTreeSet};

    let names: Vec<&String> = drift_span_names.iter().collect();
    let n = names.len();
    ...
    let mut clusters: Vec<DriftCluster> = groups
        .into_values()
        .map(|members| {
...
mod tests {
    ...
    fn singleton_with_no_shared_file_is_independent() {
        ...
        let clusters = cluster_drift_spans(&drift, &full);
        assert_eq!(
            clusters.len(),
            1,
    ...
    /// `full_anchor_paths` carries every anchored path for a span, drifted or
    /// healthy. Two drifted spans must still cluster together through a file
    /// that is currently a *healthy* (non-drifted) anchor for one of them.
    #[test]
    fn shared_file_via_a_healthy_anchor_still_clusters() {
        let drift = set(&["alpha", "beta"]);
        let mut full = HashMap::new();
        // alpha is drifted via `drift.rs`; its anchor on `shared.rs` is
    ...
    /// iterate differently from each other). `cluster_drift_spans` must
    ...
    fn ordering_is_deterministic_across_shuffled_input_maps() {
        ...
        let clusters1 = cluster_drift_spans(&drift, &full1);
        let clusters2 = cluster_drift_spans(&drift, &full2);
        assert_eq!(
            clusters1, clusters2,
        ...
        let clusters1_again = cluster_drift_spans(&drift, &full1);
