//! `git span tree` — render a clique-grouped impact tree rooted at the
...
//! authoritative prototype `scripts/git-span-tree-demo.mjs` — except the
...
/// real span graphs over a repository have at most a few thousand anchored
...
/// Run `git span tree`. Returns `Ok(0)` on success.
pub fn run_tree(repo: &gix::Repository, args: TreeArgs, span_root: &str) -> anyhow::Result<i32> {
    ...
    let span_vec: Vec<crate::types::Span> = spans.into_iter().map(|(_, m)| m).collect();
    let graph = build_graph(&span_vec);
...
/// `SpanPathIndex::matching_names_glob`); a non-glob arg uses exact equality.
...
fn resolve_roots(
    ...
    for arg in args {
        ...
        if crate::span::read::is_glob_pattern(arg) {
    ...
    if !unmatched.is_empty() {
        return Err(CliError {
            ...
            what_happened: format!(
                "These arguments matched no anchored path in any span: `{}`.",
                unmatched.join("`, `")
            ),
...
/// Co-occurrence graph: `graph[a][b]` = edge weight (product of per-span
/// occurrence counts for the pair, summed across spans). Line ranges are
/// collapsed to their file path (anchor extent ignored). Every anchored path
/// is present as a node even if it has no edges.
fn build_graph(spans: &[crate::types::Span]) -> BTreeMap<String, BTreeMap<String, u64>> {
    let mut graph: BTreeMap<String, BTreeMap<String, u64>> = BTreeMap::new();

    for span in spans {
        // Per-span occurrence count per path (line range collapsed to path).
...
/// Enumerate every maximal clique among `candidates` over the span graph
