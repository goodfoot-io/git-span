//! `git span history <span>` — chronological timeline of a span file's git
...
use crate::types::{Anchor, AnchorExtent, Span};
use anyhow::Result;
...
pub struct HistoryReport {
    ...
    /// `--limit` or `--since` dropped older span-touching commits that exist
    /// before the window. The first shown commit still diffs against the true
    /// prior span state (so its events stay truthful), but a consumer must not
...
pub struct TimelineAnchor {
    /// Combined git-span address string (`path#L<start>-L<end>` or bare path
...
/// from HEAD. Uses the same drift labels as `git span stale`.
...
pub struct CurrentAnchor {
    /// Combined git-span address string.
    pub address: String,
...
///
/// Builds a [`HistoryReport`] by walking the span file's git history, then
...
pub fn run_history(repo: &gix::Repository, args: HistoryArgs, span_root: &str) -> Result<i32> {
    ...
    let (mut commits, walk_complete) = {
        ...
        crate::git::git_log_name_only_for_paths(repo, limit, std::slice::from_ref(&span_path))?
    };
...
struct RenderedState {
    why: String,
    /// Ordered (address, extent, body). Order preserves the span's anchor
...
fn build_report(
    ...
    let current = build_current(repo, span_name, span_root)?;
...
/// engine `git span stale` uses, so the two commands agree on which anchors
...
///      span file in the working tree relative to HEAD): a worktree-added
...
///      anchor is absent from the worktree span entirely so the resolver never
...
fn build_current(
    ...
    let head = match read_span_at_in(repo, span_name, Some("HEAD"), span_root) {
        Ok(m) => Some(m),
        Err(crate::Error::SpanNotFound(_)) => None,
        Err(e) => return Err(e.into()),
    };
    let work = match read_span_at_in(repo, span_name, None, span_root) {
        Ok(m) => Some(m),
        Err(crate::Error::SpanNotFound(_)) => None,
        Err(e) => return Err(e.into()),
    };
    ...
    // Resolve the live span through the same engine `git span stale` uses:
    // worktree-over-index-over-HEAD, with the staged-span layer included so a
    ...
    for (_name, result) in resolved {
        ...
        for r in &span.anchors {
            ...
            // span's recorded anchor; the resolved `current` location carries
    ...
    // Trigger 3: detect anchors added to or removed from the worktree span
    // relative to HEAD. These are not surfaced by the resolver pass above.
    {
        // Build address sets for both sides (empty when the span is absent).
