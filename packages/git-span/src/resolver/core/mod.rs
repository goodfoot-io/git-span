...
//! used by the legacy `resolver/cache` and `resolver/cache_v2` modules that
//! this work replaced (both deleted at cutover in `0a8bf95e`). See
...
//!
//! - `resolver/cache_v2` resolved the same span set twice — once with
...
//!   top of it.
//! - Duplicate anchor addresses are valid parser input but the legacy row
//!   primary keys collapsed them (`notes/correctness-contract.md`
