//! Content-addressed FS cache for `git span stale`.
//!
//! Single polymorphic surface: a [`Cache`] backed by an L1 in-memory map and
//! an L2 on-disk store under `<common_dir>/span/cache/v1/<kind>/<aa>/<rest>`,
...
//!
//! See [`packages/git-span/plan/initial.md`](../../../plan/initial.md) for
//! the full design.
...
/// `dir`. When `enabled == false` (env `GIT_SPAN_CACHE=0` or
...
impl Cache {
    /// Open the cache rooted at `<common_dir>/span/cache/v1`. Honors
    /// `GIT_SPAN_CACHE=0` to short-circuit to a disabled cache.
