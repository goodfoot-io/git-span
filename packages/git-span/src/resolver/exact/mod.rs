...
pub(crate) fn drift_spans_new_store(
    ...
    // diagnostics event regardless of whether a later maintenance pass runs
    // (6A's `maintain` only folds it into `GcStats` when it runs).
...
fn emit_gc_stats(stats: &GcStats) {
