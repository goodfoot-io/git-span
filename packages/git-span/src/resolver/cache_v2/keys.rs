...
//! `span_tree_key`: the tree object id of the configured span root at
//! `HEAD`, or a sentinel empty-tree key when the span root does not
...
//!
//! Correctness comes from key-based invalidation: any change to span
//! root, `span_tree_key`, `source_tree_key`, `key_salt`,
...
/// `span_tree_key` when the span root is absent at `HEAD` so "no span
...
/// the HEAD-only stale answer for the whole span set.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct CommittedKey {
    pub(crate) source_tree_key: String,
    pub(crate) span_tree_key: String,
...
impl CommittedKey {
    ...
    fn digest(&self) -> [u8; 32] {
        ...
        write_prefixed(&mut h, self.span_tree_key.as_bytes());
...
/// Stable per-anchor identity within a span tree: span file path plus
/// the anchor's address. Deterministic across runs so committed and
/// dirty finding rows for the same anchor share a key.
pub(crate) fn anchor_key(span_file_path: &str, source_path: &str, start: u32, end: u32) -> String {
    let mut h = Hasher::new();
    h.update(b"gm.cache_v2.anchor-key\0");
    write_prefixed(&mut h, span_file_path.as_bytes());
...
pub(crate) struct OverlayKeyInputs {
    ...
    /// Exact staged + worktree span-file content identities.
    pub(crate) dirty_span_fingerprint: [u8; 32],
...
impl OverlayKeyInputs {
    ...
    pub(crate) fn key(&self) -> [u8; 32] {
        ...
        h.update(&self.dirty_span_fingerprint);
...
mod tests {
    ...
    fn committed_digest_sensitive_to_each_field() {
        ...
        let mut b = ck();
        b.span_tree_key = "mt2".into();
        assert_ne!(base, b.digest(), "span tree change");
        let mut c = ck();
        c.span_root = "spans".into();
        assert_ne!(base, c.digest(), "span root change");
        let mut d = ck();
    ...
    fn overlay_key_sensitive_to_each_field() {
        ...
        inp2.dirty_span_fingerprint = [2; 32];
        assert_ne!(base, inp2.key(), "dirty span");
    ...
    fn anchor_key_is_deterministic_and_distinct() {
        let a = anchor_key(".span/m", "src/a.rs", 1, 10);
        let b = anchor_key(".span/m", "src/a.rs", 1, 10);
        assert_eq!(a, b, "same inputs ⇒ same key");
        assert_ne!(a, anchor_key(".span/m", "src/a.rs", 1, 11), "range change");
        assert_ne!(a, anchor_key(".span/m", "src/b.rs", 1, 10), "path change");
        assert_ne!(a, anchor_key(".span/n", "src/a.rs", 1, 10), "span change");
    }
