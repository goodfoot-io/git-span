...
//!
//! The committed baseline is the HEAD-only resolution of every span.
//! Rendering only reports spans that have at least one non-`Fresh`
//! anchor, so the cache stores **one row per non-`Fresh` anchor**
//! (carrying enough span context to regroup) rather than a whole
//! `Vec<SpanResolved>` blob. The manifest's `complete = 1` row
//! distinguishes "built and empty" (every span Fresh) from "not built
//! yet", so a fully-clean repository is a cache hit, not a perpetual
//! rebuild.

use super::dto::{AnchorResolvedDto, SpanResolvedDto};
use super::keys::CommittedKey;
use super::schema::{CacheDb, now_secs};
use crate::types::{AnchorStatus, SpanResolved};
...
struct FindingRowPayload {
    ...
    span_message: String,
    follow_moves: bool,
    /// Position of the span in the full resolved order, so regrouping
...
impl BaselineCounts {
    pub(crate) fn from_spans(spans: &[SpanResolved]) -> Self {
        use AnchorStatus::*;
...
pub(crate) fn store_baseline(
    ...
    let counts = BaselineCounts::from_spans(spans);
    ...
    tx.execute(
        "DELETE FROM committed_stale_finding_rows \
         WHERE source_tree_key=?1 AND span_tree_key=?2 AND span_root=?3 \
...
/// A loaded committed baseline: the reportable spans (those with at
...
/// per-span anchor counts. Stored as a single blob in
...
impl WholeResultPayload {
    fn into_whole_result(self) -> Result<WholeResult> {
        ...
        for dto in self.spans {
            spans.push(dto.try_into()?);
        }
        Ok(WholeResult {
            spans,
            span_anchor_totals: self.span_anchor_totals,
        })
    }
}
...
pub(crate) fn store_whole_result(
    ...
    key: &CommittedKey,
    spans: &[SpanResolved],
    span_anchor_totals: &[(String, usize)],
) -> Result<()> {
    let filter_hex = key.filter_hex();
    let payload = WholeResultPayload {
        format_version: WHOLE_RESULT_FORMAT_VERSION,
        spans: spans.iter().map(SpanResolvedDto::from).collect(),
        span_anchor_totals: span_anchor_totals.to_vec(),
    };
    let bytes = bincode::serialize(&payload)
...
pub(crate) fn load_baseline(
    ...
    // Regroup rows into per-span resolved objects, preserving the
    // original span order captured at store time.
    ...
    let spans: Vec<SpanResolved> = grouped.into_values().collect();
