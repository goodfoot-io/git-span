//! `cache_v2` validation suite: schema round-trip, baseline/overlay
//! load↔store, key-based invalidation, and warm-clean vs warm-dirty
//! behavior at the storage layer.

use super::baseline::{
    LoadedBaseline, load_baseline, load_whole_result, store_baseline, store_whole_result,
};
use super::keys::{CommittedKey, OverlayKeyInputs, availability_hash};
use super::moved::{MovedLocation, MovedScanKey, load_scan, store_scan};
use super::overlay::{DirtyOverlay, apply_overlay, load_overlay, store_overlay};
use super::schema::{CacheDb, KEY_SALT, hex32, open_cache_at};
use crate::types::{AnchorExtent, AnchorLocation, AnchorResolved, AnchorStatus, SpanResolved};
use std::path::PathBuf;

fn tmp_db() -> (tempfile::TempDir, CacheDb) {
    let td = tempfile::tempdir().expect("tempdir");
    let db = open_cache_at(&td.path().join("span").join("stale-cache.db")).expect("open cache_v2");
    (td, db)
}

fn ck() -> CommittedKey {
    CommittedKey {
        source_tree_key: "src-tree-1".into(),
        span_tree_key: "span-tree-1".into(),
        span_root: ".span".into(),
        filter_config_hash: [3; 32],
        key_salt: KEY_SALT,
    }
}

fn anchor(id: &str, status: AnchorStatus) -> AnchorResolved {
    AnchorResolved {
        anchor_id: id.into(),
        anchor_sha: String::new(),
        anchored: AnchorLocation {
            path: PathBuf::from("src/a.rs"),
            extent: AnchorExtent::LineRange { start: 1, end: 10 },
            blob: None,
        },
        current: None,
        status,
        content_equivalent: false,
        source: None,
        layer_sources: Vec::new(),
        locus: None,
        fuzzy_successors: vec![],
    }
}

fn span(name: &str, anchors: Vec<AnchorResolved>) -> SpanResolved {
    SpanResolved {
        name: name.into(),
        message: format!("why {name}"),
        anchors,
        follow_moves: false,
    }
}

#[test]
fn baseline_round_trip_stores_only_non_fresh_rows() {
    let (_td, db) = tmp_db();
    let key = ck();
    let av = hex32(&availability_hash(true, false, false));
    let spans = vec![
        span("alpha", vec![anchor("a1", AnchorStatus::Changed)]),
        span("beta", vec![anchor("b1", AnchorStatus::Fresh)]),
    ];
    store_baseline(&db, &key, &av, &spans).expect("store");
    let loaded = load_baseline(&db, &key, &av).expect("load").expect("hit");
    // Only the span with a non-Fresh anchor is materialized.
    assert_eq!(loaded.spans.len(), 1);
    assert_eq!(loaded.spans[0].name, "alpha");
    assert_eq!(loaded.spans[0].anchors[0].status, AnchorStatus::Changed);
    assert_eq!(loaded.counts.anchors_changed, 1);
    assert_eq!(loaded.counts.anchors_fresh, 1);
    assert_eq!(loaded.non_fresh_count, 1);
}

#[test]
fn all_fresh_baseline_is_a_hit_not_a_rebuild() {
    let (_td, db) = tmp_db();
    let key = ck();
    let av = hex32(&availability_hash(true, false, false));
    let spans = vec![span("alpha", vec![anchor("a1", AnchorStatus::Fresh)])];
    store_baseline(&db, &key, &av, &spans).expect("store");
    let loaded = load_baseline(&db, &key, &av)
        .expect("load")
        .expect("complete manifest ⇒ hit even with zero finding rows");
    assert!(loaded.spans.is_empty());
    assert_eq!(loaded.non_fresh_count, 0);
}

#[test]
fn baseline_misses_on_each_invalidation_trigger() {
    let (_td, db) = tmp_db();
    let key = ck();
    let av = hex32(&availability_hash(true, false, false));
    store_baseline(
        &db,
        &key,
        &av,
        &[span("m", vec![anchor("x", AnchorStatus::Changed)])],
    )
    .expect("store");

    // span_tree_key change.
    let mut k = ck();
    k.span_tree_key = "other-span-tree".into();
    assert!(load_baseline(&db, &k, &av).unwrap().is_none());
    // source_tree_key change.
    let mut k = ck();
    k.source_tree_key = "other-src".into();
    assert!(load_baseline(&db, &k, &av).unwrap().is_none());
    // span_root change.
    let mut k = ck();
    k.span_root = "spans".into();
    assert!(load_baseline(&db, &k, &av).unwrap().is_none());
    // filter config change.
    let mut k = ck();
    k.filter_config_hash = [9; 32];
    assert!(load_baseline(&db, &k, &av).unwrap().is_none());
    // key salt change.
    let mut k = ck();
    k.key_salt = KEY_SALT + 1;
    assert!(load_baseline(&db, &k, &av).unwrap().is_none());
    // availability change.
    let av2 = hex32(&availability_hash(false, false, false));
    assert!(load_baseline(&db, &key, &av2).unwrap().is_none());

    // The original key still hits.
    assert!(load_baseline(&db, &key, &av).unwrap().is_some());
}

#[test]
fn overlay_round_trip_and_merge_replaces_affected_span() {
    let (_td, db) = tmp_db();
    let key = ck();
    let av = hex32(&availability_hash(true, false, false));

    let baseline_spans = vec![
        span("alpha", vec![anchor("a1", AnchorStatus::Changed)]),
        span("beta", vec![anchor("b1", AnchorStatus::Changed)]),
    ];
    store_baseline(&db, &key, &av, &baseline_spans).expect("store");
    let loaded = load_baseline(&db, &key, &av).unwrap().unwrap();

    // Overlay re-resolves `alpha` to Deleted; `beta` is untouched.
    let mut inputs = OverlayKeyInputs::new(&key);
    inputs.dirty_source_fingerprint = [1; 32];
    let overlay = DirtyOverlay {
        affected_spans: vec!["alpha".into()],
        spans: vec![span("alpha", vec![anchor("a1", AnchorStatus::Deleted)])],
    };
    store_overlay(&db, &inputs, &overlay).expect("store overlay");

    let reloaded = load_overlay(&db, &inputs)
        .expect("load overlay")
        .expect("overlay hit");
    let merged = apply_overlay(&loaded.spans, &reloaded);
    let alpha = merged.iter().find(|m| m.name == "alpha").unwrap();
    assert_eq!(alpha.anchors[0].status, AnchorStatus::Deleted);
    let beta = merged.iter().find(|m| m.name == "beta").unwrap();
    assert_eq!(beta.anchors[0].status, AnchorStatus::Changed);
}

#[test]
fn overlay_misses_on_dirty_identity_change() {
    let (_td, db) = tmp_db();
    let key = ck();
    let mut inputs = OverlayKeyInputs::new(&key);
    inputs.dirty_source_fingerprint = [1; 32];
    store_overlay(
        &db,
        &inputs,
        &DirtyOverlay {
            affected_spans: vec!["m".into()],
            spans: vec![span("m", vec![anchor("x", AnchorStatus::Changed)])],
        },
    )
    .expect("store");
    assert!(load_overlay(&db, &inputs).unwrap().is_some());

    let mut changed = OverlayKeyInputs::new(&key);
    changed.dirty_source_fingerprint = [2; 32];
    assert!(
        load_overlay(&db, &changed).unwrap().is_none(),
        "different dirty content identity ⇒ overlay miss"
    );
}

#[test]
fn overlay_affected_but_now_fresh_span_is_dropped() {
    let (_td, db) = tmp_db();
    let key = ck();
    let av = hex32(&availability_hash(true, false, false));
    store_baseline(
        &db,
        &key,
        &av,
        &[span("m", vec![anchor("x", AnchorStatus::Changed)])],
    )
    .unwrap();
    let loaded = load_baseline(&db, &key, &av).unwrap().unwrap();

    let mut inputs = OverlayKeyInputs::new(&key);
    inputs.dirty_source_fingerprint = [7; 32];
    // Affected but no reportable form ⇒ the anchor went back to Fresh.
    let overlay = DirtyOverlay {
        affected_spans: vec!["m".into()],
        spans: Vec::new(),
    };
    store_overlay(&db, &inputs, &overlay).unwrap();
    let reloaded = load_overlay(&db, &inputs).unwrap().unwrap();
    let merged = apply_overlay(&loaded.spans, &reloaded);
    assert!(
        merged.iter().all(|m| m.name != "m"),
        "an affected span with no overlay finding is dropped from output"
    );
}

#[test]
fn moved_scan_negative_result_is_a_hit() {
    let (_td, db) = tmp_db();
    let key = MovedScanKey {
        source_tree_key: "src1".into(),
        filter_config_hash_hex: "ff".into(),
        hash_algorithm: git_span_core::RK64_ALGORITHM.into(),
        content_hash: "deadbeef".into(),
        extent_kind: "line-range".into(),
        line_count: 10,
    };
    assert!(
        load_scan(&db, &key).unwrap().is_none(),
        "no manifest ⇒ miss"
    );
    store_scan(&db, &key, &[]).unwrap();
    let hit = load_scan(&db, &key)
        .unwrap()
        .expect("complete manifest with zero rows is a hit");
    assert!(hit.is_empty());
}

#[test]
fn moved_scan_round_trip_and_key_isolation() {
    let (_td, db) = tmp_db();
    let key = MovedScanKey {
        source_tree_key: "src1".into(),
        filter_config_hash_hex: "ab".into(),
        hash_algorithm: git_span_core::RK64_ALGORITHM.into(),
        content_hash: "cafe".into(),
        extent_kind: "line-range".into(),
        line_count: 5,
    };
    let locs = vec![MovedLocation {
        source_path: "src/moved.rs".into(),
        start_line: 20,
        end_line: 24,
    }];
    store_scan(&db, &key, &locs).unwrap();
    assert_eq!(load_scan(&db, &key).unwrap().unwrap(), locs);

    let mut other = key.clone();
    other.content_hash = "feed".into();
    assert!(
        load_scan(&db, &other).unwrap().is_none(),
        "different content hash ⇒ independent scan key"
    );
}

#[test]
fn default_loaded_baseline_is_empty() {
    let b = LoadedBaseline::default();
    assert!(b.spans.is_empty());
    assert_eq!(b.non_fresh_count, 0);
}

// ── Whole-result cache tests ──────────────────────────────────────────

#[test]
fn whole_result_roundtrip() {
    let (_td, db) = tmp_db();
    let key = ck();
    let spans = vec![
        span(
            "alpha",
            vec![
                anchor("a1", AnchorStatus::Changed),
                anchor("a2", AnchorStatus::Fresh),
            ],
        ),
        span("beta", vec![anchor("b1", AnchorStatus::Fresh)]),
    ];
    let totals: Vec<(String, usize)> = vec![("alpha".into(), 2), ("beta".into(), 1)];

    store_whole_result(&db, &key, &spans, &totals).expect("store");
    let loaded = load_whole_result(&db, &key).expect("load").expect("hit");

    assert_eq!(loaded.spans.len(), 2, "both spans stored");
    assert_eq!(loaded.spans[0].name, "alpha");
    assert_eq!(loaded.spans[0].anchors.len(), 2, "alpha: both anchors backfilled");
    assert_eq!(loaded.spans[1].name, "beta");
    assert_eq!(loaded.spans[1].anchors.len(), 1, "beta: fresh anchor backfilled");
    assert_eq!(loaded.span_anchor_totals.len(), 2);
    assert_eq!(loaded.span_anchor_totals[0], ("alpha".to_string(), 2));
    assert_eq!(loaded.span_anchor_totals[1], ("beta".to_string(), 1));
}

#[test]
fn whole_result_key_mismatch_returns_none() {
    let (_td, db) = tmp_db();
    let key = ck();
    let spans = vec![span("m", vec![anchor("a", AnchorStatus::Fresh)])];
    let totals: Vec<(String, usize)> = vec![("m".into(), 1)];

    store_whole_result(&db, &key, &spans, &totals).expect("store");

    // Different source_tree_key → miss
    let mut other = ck();
    other.source_tree_key = "different-source-tree".into();
    assert!(
        load_whole_result(&db, &other).expect("load").is_none(),
        "different source_tree_key ⇒ miss"
    );

    // Different span_tree_key → miss
    let mut other2 = ck();
    other2.span_tree_key = "different-span-tree".into();
    assert!(
        load_whole_result(&db, &other2).expect("load").is_none(),
        "different span_tree_key ⇒ miss"
    );
}

#[test]
fn whole_result_empty_span_root() {
    let (_td, db) = tmp_db();
    let key = ck();
    let spans: Vec<SpanResolved> = Vec::new();
    let totals: Vec<(String, usize)> = Vec::new();

    store_whole_result(&db, &key, &spans, &totals).expect("store");
    let loaded = load_whole_result(&db, &key).expect("load").expect("hit");

    assert!(loaded.spans.is_empty(), "no spans");
    assert!(loaded.span_anchor_totals.is_empty(), "no totals");
}

#[test]
fn whole_result_key_salt_mismatch_returns_none() {
    let (_td, db) = tmp_db();
    let key = ck();
    let spans = vec![span("m", vec![anchor("a", AnchorStatus::Fresh)])];
    let totals: Vec<(String, usize)> = vec![("m".into(), 1)];

    store_whole_result(&db, &key, &spans, &totals).expect("store");

    // Different key_salt → miss
    let mut other = ck();
    other.key_salt = KEY_SALT + 1;
    assert!(
        load_whole_result(&db, &other).expect("load").is_none(),
        "different key_salt ⇒ miss"
    );
}
