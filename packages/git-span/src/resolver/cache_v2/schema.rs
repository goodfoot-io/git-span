...
//!
//! The database lives at `$GIT_COMMON_DIR/span/stale-cache.db` and is
...
/// Database file basename under `<common_dir>/span/`.
...
pub(crate) fn db_path(repo: &gix::Repository) -> PathBuf {
    crate::git::common_dir(repo).join("span").join(DB_BASENAME)
}
...
CREATE TABLE IF NOT EXISTS span_anchor_rows (
  span_tree_key  TEXT NOT NULL,
  span_root      TEXT NOT NULL,
  key_salt       INTEGER NOT NULL,
  anchor_key     TEXT NOT NULL,
  span_name      TEXT NOT NULL,
  span_file_path TEXT NOT NULL,
  span_file_line INTEGER NOT NULL,
  ...
  PRIMARY KEY (span_tree_key, span_root, key_salt, anchor_key)
) STRICT;
...
CREATE TABLE IF NOT EXISTS anchor_manifest (
  ...
  span_file_count INTEGER NOT NULL,
  complete        INTEGER NOT NULL,
  created_at      INTEGER NOT NULL,
  PRIMARY KEY (span_tree_key, span_root, key_salt, parser_version)
) STRICT;

CREATE INDEX IF NOT EXISTS span_anchor_rows_by_span_file
ON span_anchor_rows (span_tree_key, span_root, key_salt, span_file_path);

CREATE INDEX IF NOT EXISTS span_anchor_rows_by_path
ON span_anchor_rows (span_tree_key, span_root, key_salt, source_path);
...
CREATE TABLE IF NOT EXISTS committed_stale_finding_rows (
  ...
  created_at         INTEGER NOT NULL,
  PRIMARY KEY (
    source_tree_key, span_tree_key, span_root,
...
CREATE TABLE IF NOT EXISTS committed_stale_summary (
  ...
  created_at         INTEGER NOT NULL,
  PRIMARY KEY (
    source_tree_key, span_tree_key, span_root, filter_config_hash, key_salt
  )
) STRICT;
...
CREATE TABLE IF NOT EXISTS committed_stale_whole_result (
  ...
  payload            BLOB NOT NULL,
  created_at         INTEGER NOT NULL,
  PRIMARY KEY (source_tree_key, span_tree_key, span_root, filter_config_hash, key_salt)
) STRICT;
"#;
...
/// can change a resolution's output without HEAD or the span tree
