//! Dirty overlay: row-level findings for only the spans whose anchors
...
//! span (its reportable anchors) so merging onto the committed baseline
//! is a per-span replace: an affected span takes its overlay value, an
//! unaffected span keeps its baseline value. The `dirty_overlay_manifest`
...
/// One overlay finding row: a non-`Fresh` anchor of an affected span,
/// plus the span context needed to regroup it.
...
struct OverlaySummaryPayload {
    format_version: u8,
    /// Names of every span the overlay re-resolved, even those that
    /// became all-`Fresh` (so the merge knows to drop them).
    affected_spans: Vec<String>,
}

/// In-memory overlay: the affected span set + their re-resolved
/// reportable form.
#[derive(Clone, Debug, Default, PartialEq)]
pub(crate) struct DirtyOverlay {
    pub(crate) affected_spans: Vec<String>,
    pub(crate) spans: Vec<SpanResolved>,
}
...
pub(crate) fn store_overlay(
    ...
    for (order, span) in overlay.spans.iter().enumerate() {
        for a in &span.anchors {
            ...
            let payload = OverlayRowPayload {
                ...
                span_message: span.message.clone(),
                follow_moves: span.follow_moves,
                span_order: order as u32,
    ...
    let summary = OverlaySummaryPayload {
        format_version: FORMAT_VERSION,
        affected_spans: overlay.affected_spans.clone(),
    };
...
pub(crate) fn load_overlay(
    ...
    let affected_spans = match summary {
        Some(bytes) => match bincode::deserialize::<OverlaySummaryPayload>(&bytes) {
            Ok(s) if s.format_version == FORMAT_VERSION => s.affected_spans,
            _ => return Ok(None),
        },
        None => return Ok(None),
    };
    ...
    let mut grouped: std::collections::BTreeMap<(u32, String), SpanResolved> =
    ...
    for row in rows {
        ...
        grouped
            .entry((payload.span_order, payload.span_name.clone()))
            .or_insert_with(|| SpanResolved {
                name: payload.span_name.clone(),
                message: payload.span_message.clone(),
                anchors: Vec::new(),
    ...
    Ok(Some(DirtyOverlay {
        affected_spans,
        spans: grouped.into_values().collect(),
    }))
}

/// Merge `overlay` onto `baseline_spans` (the reportable committed
/// spans). A span named in `affected_spans` takes its overlay value
/// (or is dropped if the overlay produced no reportable form); every
/// other span keeps its baseline value. Baseline order is preserved;
/// overlay-only spans are appended.
pub(crate) fn apply_overlay(
    baseline_spans: &[SpanResolved],
    overlay: &DirtyOverlay,
) -> Vec<SpanResolved> {
    let affected: HashSet<&str> = overlay.affected_spans.iter().map(|s| s.as_str()).collect();
    let overlay_by_name: std::collections::HashMap<&str, &SpanResolved> = overlay
        .spans
        .iter()
        .map(|m| (m.name.as_str(), m))
        .collect();
    let mut out: Vec<SpanResolved> =
        Vec::with_capacity(baseline_spans.len() + overlay.spans.len());
    let mut seen: HashSet<String> = HashSet::new();
    for m in baseline_spans {
        if affected.contains(m.name.as_str()) {
            seen.insert(m.name.clone());
            if let Some(rep) = overlay_by_name.get(m.name.as_str()) {
                out.push((*rep).clone());
            }
            // Affected but no reportable overlay form ⇒ span became
            // all-Fresh; drop it from the rendered set.
            continue;
        }
        out.push(m.clone());
    }
    for m in &overlay.spans {
