...
//! cache. It is the single live cache path for `git span stale`. There
...
//! uncached resolver (`stale_spans_inner`), never to old rows.
//!
//! ## Keys
//!
//! `span_tree_key` is the tree object id of the configured span root at
//! `HEAD` (or the empty-tree sentinel when absent), replacing the
//! removed `catalog_tree_oid`. The committed key is
//! `(source_tree_key, span_tree_key, span_root, filter_config_hash,
//! key_salt)`; the dirty overlay key folds in exact staged/worktree
//! content identities and the index checksum.
//!
//! ## Paths
//!
//! * **Warm clean** — keys resolve, no dirty span/source files, the
...
//!   dirty paths/span files, load-or-build the row-level overlay, merge,
//!   render.
...
/// Tree object id of the span root at `HEAD`, or the empty-tree
/// sentinel when the span root is absent at `HEAD`.
fn span_tree_key(repo: &gix::Repository, span_root: &str) -> Result<String> {
    match crate::git::tree_entry_at(repo, "HEAD", std::path::Path::new(span_root))? {
...
/// Attempt to satisfy `git span stale` from the file-backed cache.
///
/// Returns [`CacheAttempt::Resolved`] with the reportable span set on a
...
pub(crate) fn stale_spans_cached(
    repo: &gix::Repository,
    span_root: &str,
    options: EngineOptions,
) -> Result<CacheAttempt> {
    let _perf = crate::perf::span("resolver.cache_v2");
    if std::env::var("GIT_SPAN_CACHE_V2").as_deref() == Ok("0") {
    ...
    // (F1) — not a hardcoded `.span`.
    let span_root = span_root.to_string();
    ...
    let committed = CommittedKey {
        ...
        span_tree_key: span_tree_key(repo, &span_root)?,
        span_root: span_root.clone(),
    ...
    let corpus_has_lfs = corpus_has_lfs(repo, &span_root);
    let availability =
    ...
    let baseline = {
        ...
        match load_baseline(&db, &committed, &availability_hex) {
            ...
            Ok(None) => {
                ...
                let spans = match build_committed_spans(repo, &span_root) {
                    Ok(m) => m,
                    Err(e) => {
                        return Ok(CacheAttempt::Fallback(format!("build-baseline: {e}")));
                    }
                };
                if let Err(e) = store_baseline(&db, &committed, &availability_hex, &spans) {
    ...
    // Uncommitted span files that `git status -uno` never reports — every
    // worktree span file absent from HEAD (untracked, including
    // gitignored). The committed baseline only contains HEAD spans, so
    // these must enter the dirty-overlay path to be surfaced at all; and
    // marking them dirty is what later lets a deleted untracked span drop
    ...
    if clean {
        ...
        // resolution (`stale_spans_inner`), across Human, JSON, and
        ...
        // coincide: an effective resolution over the committed span names
        ...
        let whole_result = match baseline.whole_result {
            Some(wr) => Some(wr),
            None => {
                match build_clean_whole_result(repo, &span_root, options) {
        ...
        let spans = match &whole_result {
            Some(wr) => reportable(wr.spans.clone()),
            None => reportable(baseline.spans),
        };
        return Ok(CacheAttempt::Resolved {
            spans,
            whole_result,
    ...
    crate::perf::counter("cache_v2.affected-spans", affected_spans.len() as u64);

    // Overlay key: committed digest + index checksum + exact dirty
    // span-file and source-file content identities + layer identity.
    let workdir = crate::git::work_dir(repo)?;
    // Each dirty source file's identity is a digest of its worktree
    // bytes (the same content identity dirty span files use below), not
    ...
    let dirty_span_ids: Vec<(String, String)> = affected_spans
        .iter()
        .map(|n| {
            let rel = format!("{span_root}/{n}");
            let id = file_content_identity(workdir, &rel);
            (rel, id)
        })
        .collect();
    let dirty_span_fp = content_identity_fingerprint(
        b"gm.cache_v2.dirty-span\0",
        dirty_span_ids
            .iter()
            .map(|(p, i)| (p.as_str(), i.as_str()))
            .collect::<Vec<_>>(),
    ...
    overlay_inputs.dirty_span_fingerprint = dirty_span_fp;
    // Layer identity: full effective view; tombstone state folds into
    // the affected-span identities above. A constant domain marks the
    ...
    let overlay = {
        ...
        match load_overlay(&db, &overlay_inputs) {
            ...
            Ok(None) => {
                ...
                let names: Vec<String> = affected_spans.iter().cloned().collect();
                let spans = if names.is_empty() {
                    Vec::new()
                } else {
                    match super::engine::resolve_named_spans(repo, &span_root, &names, options) {
                        Ok(resolved) => {
                            let mut out = Vec::new();
    ...
    // The render base here covers the UNAFFECTED spans — the ones whose
    // anchors no dirty path touches. For such a span the worktree content
    ...
    // closes it. The within-span ordering divergence the SQLite-regrouped
    ...
    // with the dirty-span count, not corpus size.
    //
    ...
    // drift into a committed-keyed entry (an F1-class bug). The affected-span
    // OVERLAY is still authoritative for spans in the dirty set (its key
    // folds in exact worktree content identities and the index checksum), so
    // we overlay it on top — and worktree-only (uncommitted) spans, which
    // have no committed record, come solely from the overlay.
    let committed_base = match build_committed_render_base(repo, &span_root, &baseline.spans) {
        Ok(spans) => spans,
        Err(e) => {
            return Ok(CacheAttempt::Fallback(format!(
    ...
    Ok(CacheAttempt::Resolved {
        spans: reportable(merged),
        whole_result: None,
    })
}

/// Build the warm-clean whole-result by resolving the committed spans with
/// the EFFECTIVE layer set, then re-shaping each span's anchor list to the
/// committed record. Returns `Ok(None)` only when there are no committed
/// spans; otherwise `Ok(Some((whole_result, has_interior_anchor)))`.
///
...
/// cache-off path (`stale_spans_inner`) by construction. This is what makes
/// the cached `current.blob` (F3) and interior-anchor drift label (F2) match
/// cache-off, where a `committed_only` resolution diverges.
///
/// Anchor DEFINITIONS still come from the committed span records (F1): each
/// resolved span's anchors are rebuilt from its committed record, keeping the
...
fn build_clean_whole_result(
    ...
    let file_records = committed_span_records(repo, span_root)?;
    ...
    let has_interior_anchor = file_records.values().any(|m| {
        m.anchors
            .iter()
            .any(|(_, a)| crate::span_root::classify_interior_anchor(span_root, &a.path).is_some())
    });

    let span_anchor_totals: Vec<(String, usize)> = file_records
        .iter()
        .map(|(n, m)| (n.clone(), m.anchors.len()))
        .collect();

    // Effective resolution over the committed span names. `options` is the
    // full layer set (the cache-eligibility gate rejects any non-full set),
    // so this mirrors `stale_spans_inner`'s source resolution exactly.
    let names: Vec<String> = file_records.keys().cloned().collect();
    let resolved = super::engine::resolve_named_spans(repo, span_root, &names, options)?;
    let mut resolved_by_name: std::collections::HashMap<String, SpanResolved> =
    ...
    // Re-shape to the committed record (F1): one resolved span per committed
    // record, in committed-name order, anchors driven by the committed record.
    let mut backfilled: Vec<SpanResolved> = Vec::with_capacity(file_records.len());
    for (name, record) in &file_records {
        ...
        for (anchor_id, a) in &record.anchors {
            match resolved.and_then(|m| m.anchors.iter().find(|r| &r.anchor_id == anchor_id)) {
                ...
                None => rebuilt.push(fresh_anchor_resolved_from_span(anchor_id, a)),
            }
        }
...
/// (worktree-layer-on) equivalent for an UNAFFECTED span.
///
...
/// [`resolve_named_spans`]. Mirrors the SHAPE of [`build_clean_whole_result`]
/// — one resolved span per committed record from [`committed_span_records`],
/// anchors in committed-record order, `Fresh` synthesized via
/// [`fresh_anchor_resolved_from_span`] — but draws each non-`Fresh` anchor from
/// `baseline_spans` (the loaded `committed_only` findings) normalized via
/// [`normalize_committed_anchor`], paying NO resolution cost.
///
/// The baseline stores only non-`Fresh` findings, regrouped by `(span_order,
...
/// baseline's stored order. The unaffected spans this produces are correct as
/// rendered; the affected (dirty) spans are replaced by `apply_overlay`, which
/// supplies the authoritative effective re-resolution.
fn build_committed_render_base(
    repo: &gix::Repository,
    span_root: &str,
    baseline_spans: &[SpanResolved],
    ...
    let baseline_by_name: std::collections::HashMap<&str, &SpanResolved> = baseline_spans
        .iter()
...
/// Synthesize a `Fresh` `AnchorResolved` from a span-file anchor record,
/// matching the shape produced by [`fresh_anchor_resolved`] in the CLI layer.
fn fresh_anchor_resolved_from_span(
    anchor_id: &str,
...
/// Keep only spans that have a non-`Fresh` anchor.
/// the same predicate the engine uses for stale discovery output.
fn reportable(spans: Vec<SpanResolved>) -> Vec<SpanResolved> {
    spans
        .into_iter()
        .filter(|m| {
            m.anchors
...
/// Span-relative paths (`<span_root>/<name>`) of worktree span files that
/// are not committed at `HEAD` — untracked files, including gitignored
/// ones, which `git status -uno` does not enumerate.
///
/// Returned in `<span_root>/<name>` form so they slot directly into the
/// `dirty_paths` set, where [`spans_affected_by`] turns the span-root
/// prefix back into a span name.
fn uncommitted_span_paths(repo: &gix::Repository, span_root: &str) -> Result<Vec<String>> {
    let reader = crate::span_file_reader::SpanFileReader::new(repo, span_root.to_string());
    let committed: HashSet<String> = reader.committed_span_names()?.into_iter().collect();
    Ok(reader
        .worktree_span_names()?
        .into_iter()
        .filter(|name| !committed.contains(name))
        .map(|name| format!("{span_root}/{name}"))
        .collect())
}

/// Full HEAD-only resolution of every span committed at `HEAD` — the
/// cold-build input for the committed baseline.
///
/// When more than one span exists and multiple CPUs are available, spans
...
fn build_committed_spans(repo: &gix::Repository, span_root: &str) -> Result<Vec<SpanResolved>> {
    let _perf = crate::perf::span("resolver.cache_v2.build-baseline");
    // The committed baseline is keyed by the HEAD span tree and resolved
    // with `committed_only`, so it must enumerate exactly the spans that
    // exist at HEAD. Enumerating the worktree filesystem (the old
    // behavior) baked untracked/gitignored span files into a HEAD-keyed
    // baseline; deleting such a file changed neither the key nor `git
    // status -uno`, so the warm-clean path replayed the deleted span.
    // Worktree-only spans are uncommitted state handled by the
    // dirty-overlay path below.
    let reader = crate::span_file_reader::SpanFileReader::new(repo, span_root.to_string());
    let names: Vec<String> = reader.committed_span_names()?;
    if names.is_empty() {
        return Ok(Vec::new());
    }

    // Single-span or single-CPU: resolve serially.
    let cpus = std::thread::available_parallelism()
        .map(|n| n.get())
        .unwrap_or(1);
    if names.len() <= 1 || cpus <= 1 {
        let resolved = super::engine::resolve_named_spans(
            repo,
            span_root,
            &names,
            EngineOptions {
    ...
    let resolved = super::engine::resolve_named_spans_parallel(
        repo,
        span_root,
        &names,
        options,
...
/// Committed span records (`name` → `Span`) read from the HEAD layer only.
///
/// Used to backfill the whole-result cache entry's per-span anchor sets and
/// `span_anchor_totals` from committed content, so the stored entry matches
...
/// commit).
fn committed_span_records(
    repo: &gix::Repository,
    span_root: &str,
) -> Result<std::collections::HashMap<String, crate::types::Span>> {
    let reader = crate::span_file_reader::SpanFileReader::new(repo, span_root.to_string());
    ...
    for name in names {
        if let Some(file) = reader.read_head(&name)? {
            out.insert(name.clone(), crate::types::span_from_file(&name, &file));
        }
    }
    Ok(out)
}

/// Span names with at least one anchor whose source path is dirty, plus
/// any span whose own span file is dirty (so a span-file edit
/// re-resolves that span).
fn spans_affected_by(
    repo: &gix::Repository,
    dirty_paths: &BTreeSet<String>,
    span_root: &str,
) -> Result<BTreeSet<String>> {
    let mut affected: BTreeSet<String> = BTreeSet::new();
    let prefix = format!("{span_root}/");
    ...
    if !dirty_paths.is_empty() {
        for (name, span) in crate::span::read::load_all_spans_in(repo, span_root)?.0 {
            if span
                .anchors
                .iter()
...
/// whenever a dirty span file's content changes.
