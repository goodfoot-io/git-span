 ...
 *
 * One-shot migration for git-span repos created before the Range→Anchor
 * rename.  Converts:
 *   - refs/ranges/v1/<uuid>  →  refs/anchors/v1/<uuid>
 *   - blob header:  "anchor <sha>" → "commit <sha>",
 *                   "range <start> <end> <blob>\t<path>" → "extent ..."
 *   - span tree entry:  "ranges" file  →  "anchors" file  (same content)
 *
...
// ── 4. Rewrite span trees that reference a "ranges" file ──────────────────

const spanRefsRaw = git(['for-each-ref', '--format=%(refname)', 'refs/spans/v1']).trim();
const spanRefs = spanRefsRaw ? spanRefsRaw.split('\n') : [];

let spansRewritten = 0;

for (const spanRef of spanRefs) {
  if (!spanRef) continue;

  // Get the commit SHA the span ref points at.
  const spanCommitSha = git(['rev-parse', spanRef]).trim();

  // List top-level tree entries of the span commit.
  const treeEntries = git(['ls-tree', spanCommitSha]).trim();
  ...
  if (hasAnchorsFile) {
    ...
    die(`span ${spanRef} has both "ranges" and "anchors" tree entries — cannot migrate automatically`);
  }

  if (dryRun) {
    log(`[dry-run] would rewrite span tree for ${spanRef} (rename "ranges" → "anchors")`);
    spansRewritten++;
    continue;
  }
  ...
  const commitMessage = git(['log', '-1', '--format=%B', spanCommitSha]).trimEnd();
  const authorName = git(['log', '-1', '--format=%an', spanCommitSha]).trim();
  const authorEmail = git(['log', '-1', '--format=%ae', spanCommitSha]).trim();
  const authorDate = git(['log', '-1', '--format=%ai', spanCommitSha]).trim();
  const committerName = git(['log', '-1', '--format=%cn', spanCommitSha]).trim();
  const committerEmail = git(['log', '-1', '--format=%ce', spanCommitSha]).trim();
  const committerDate = git(['log', '-1', '--format=%ci', spanCommitSha]).trim();

  // Get parent commits (there should be one for normal span commits).
  const parentsRaw = git(['log', '-1', '--format=%P', spanCommitSha]).trim();
  ...
  try {
    ...
    const msg = err.stderr ? err.stderr.toString().trim() : String(err);
    die(`git commit-tree for ${spanRef}: ${msg}`);
  }

  git(['update-ref', spanRef, newCommitSha]);
  spansRewritten++;
}
...
const m = spansRewritten;
if (dryRun) {
  log(`[dry-run] would migrate ${n} anchor${n !== 1 ? 's' : ''} across ${m} span${m !== 1 ? 'es' : ''}`);
} else {
  log(`migrated ${n} anchor${n !== 1 ? 's' : ''} across ${m} span${m !== 1 ? 'es' : ''}`);
}
