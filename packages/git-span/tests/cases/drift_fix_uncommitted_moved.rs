//! Regression test for `git span drift --fix` silently skipping exact
//! in-file `Moved` anchors whose drift lives only in the worktree or
//! only in the index (uncommitted).
//!
//! `--help` for `drift --fix` promises "Re-anchor `Moved` anchors
...
//! gate (`packages/git-span/src/cli/drift_fix.rs`, around L629-643)
...
//! anchor's recorded position is drifted. The branch that actually
//! performs the exact in-file `Moved` fix (drift_fix.rs, around
...
//! `fix_rewrites_moved_anchor_at_worktree_layer` in `cli_drift_fix.rs`
...
fn fix_rewrites_moved_anchor_at_worktree_only_layer() -> Result<()> {
    ...
    assert_eq!(
        post.status.code(),
        Some(0),
        "drift must be clean after --fix reconciled the worktree-only move; stdout={}",
...
fn fix_rewrites_moved_anchor_at_index_only_layer() -> Result<()> {
    ...
    assert_eq!(
        post.status.code(),
        Some(0),
        "drift must be clean after --fix reconciled the index-only move; stdout={}",
        String::from_utf8_lossy(&post.stdout)
    );
    Ok(())
}
