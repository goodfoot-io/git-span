...
fn nul_bearing_file_rejects_line_range_add() -> Result<()> {
    ...
    let out = repo.run_span(["add", "m", "data.bin#L1-L1"])?;
    assert!(
        !out.status.success(),
    ...
    let out2 = repo.run_span(["add", "m", "data.bin"])?;
    assert!(
        out2.status.success(),
...
fn non_utf8_file_rejects_line_range_add() -> Result<()> {
    ...
    let out = repo.run_span(["add", "m", "image.bin#L1-L1"])?;
    assert!(
        !out.status.success(),
    ...
    let whole = repo.run_span(["add", "m", "image.bin"])?;
    assert!(
