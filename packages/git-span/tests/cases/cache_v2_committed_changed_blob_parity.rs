...
//! the effective cache-off resolver (`stale_spans_inner`) leaves it `None`.
//!
...
/// Seed a span anchoring `a.txt#L1-L3`, then drift that region via a COMMIT
...
fn seed_committed_changed(repo: &TestRepo) -> Result<()> {
    ...
    repo.run_span(["add", "m", "a.txt#L1-L3"])?;
    repo.run_span(["why", "m", "-m", "anchor on a"])?;
