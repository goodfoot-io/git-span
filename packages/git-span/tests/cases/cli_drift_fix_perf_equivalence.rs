//! Output-equivalence guard for `git span drift --fix`.
//!
...
/// Run `git span drift --fix` (with optional extra args) and return
...
//
// Runs `git span drift --fix` (no positional span), captures
...
fn equivalence_warning_stderr_parity() -> Result<()> {
    ...
    // anchor is terminal.
    {
        ...
        cmd.args(["drift", "--fix", "--no-exit-code"]);
        cmd.output()?;
    }
...
fn golden_cold_path_bare_scan() -> Result<()> {
    ...
    // bare scan (drift report) shows no spans and prints the 0-drift summary.
    assert_eq!(
        code,
        Some(0),
    ...
    assert_eq!(
        stdout,
        "0 drift across 1 span (1 anchor checked)\n\
...
fn golden_named_scope_single_warning() -> Result<()> {
    ...
    let run = || -> Result<(Vec<u8>, Option<i32>)> {
        ...
        cmd.current_dir(repo.path());
        cmd.env("GIT_SPAN_RENAME_BUDGET", "0");
        cmd.args(["drift", "--fix", "warn-span", "--no-exit-code"]);
        let out = cmd.output()?;
...
/// Named-scope `git span drift [span...]` with two spans sharing an identical
...
/// edit drifts both so they surface as drifted.  Running `drift zzz aaa` must
/// render `## zzz` before `## aaa`; running `drift aaa zzz` must render
/// `## aaa` before `## zzz`.  The same assertion holds for `drift --fix`.
#[test]
fn golden_near_tie_ordering_stable() -> Result<()> {
    ...
    assert!(
        pz3 < pa3,
        "drift --fix zzz-tie aaa-tie: expected zzz-tie before aaa-tie, got:\n{s_fix_za}"
    );
    ...
    assert!(
        pa4 < pz4,
        "drift --fix aaa-tie zzz-tie: expected aaa-tie before zzz-tie, got:\n{s_fix_az}"
    );
