//! Reproduction test: `git span drift` must emit `ContentUnavailable(SparseExcluded)`
...
//! 2. Commit the file, anchor it with `git span add`, commit the span.
//! 3. Enable sparse-checkout (non-cone mode) to exclude the anchored file
//!    but keep `.span/` accessible.
//! 4. Run `git span drift --format=json`.
...
fn sparse_checkout_excluded_reports_content_unavailable() -> Result<()> {
    ...
    // while keeping the .span/ directory in the root.
    ...
    // Enable sparse-checkout in non-cone mode, keeping only .span/.
    ...
    // Sanity: the span file is still readable (it matches .span/).
    assert!(
        repo.path().join(".span/m").exists(),
        ".span/m must still be on disk after sparse-checkout"
    );

    // Run drift -- this SHOULD report SPARSE_EXCLUDED against the fixed code.
