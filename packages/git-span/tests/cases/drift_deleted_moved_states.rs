...
//!
//! - F2: `git span drift` works on an ordinary repo with no commit-graph.
...
//! - F7: `git span drift --stat` lists only drifted anchors and the
...
use git_span::{resolve_span, drift_spans};
...
/// commit-graph. `drift` must still succeed (no plumbing instruction as a
/// fatal error).
#[test]
fn drift_succeeds_without_commit_graph() -> Result<()> {
    ...
    let _ = drift_spans(&gix, ".span", full_opts())?;

    // And via the CLI binary.
    let out = repo.run_span(vec!["drift"])?;
    assert!(
        out.status.success(),
        "`git span drift` failed on a no-commit-graph repo: {}",
...
fn whole_file_path_removed_reads_deleted() -> Result<()> {
    ...
    let drift = repo.span_stdout(["drift", "wf", "--no-exit-code"])?;
    assert!(
        drift.contains("deleted in the index") || drift.contains("deleted in the working tree"),
        "git rm of an anchored whole-file path must read as a deletion, \
         not 'changed'; drift=\n{drift}"
    );
    assert!(
        !drift.contains("— changed"),
        "removal must never be labeled 'changed'; drift=\n{drift}"
    );
    Ok(())
}
...
fn line_range_path_removed_reads_deleted() -> Result<()> {
    ...
    let drift = repo.span_stdout(["drift", "lr", "--no-exit-code"])?;
    assert!(
        drift.contains("deleted in the index") || drift.contains("deleted in the working tree"),
...
fn human_renamed_deletion_reports_re_anchor_target() -> Result<()> {
    ...
    let out = repo.span_stdout(["drift", "wf", "--no-exit-code"])?;
    assert!(
...
fn json_renamed_deletion_has_renamed_at_and_renamed_to_fields() -> Result<()> {
    let repo = TestRepo::new()?;
    seed_renamed_whole_file(&repo)?;
    let out = repo.span_stdout(["drift", "wf", "--format=json", "--no-exit-code"])?;
...
fn porcelain_renamed_deletion_emits_renamed_to_comment() -> Result<()> {
    let repo = TestRepo::new()?;
    seed_renamed_whole_file(&repo)?;
    let out = repo.span_stdout(["drift", "wf", "--format=porcelain", "--no-exit-code"])?;
