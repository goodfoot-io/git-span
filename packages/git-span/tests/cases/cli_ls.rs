...
fn span_stdout_with_stdin(repo: &TestRepo, args: &[&str], stdin: &str) -> Result<String> {
...
fn ls_batch_porcelain_emits_hit_rows() -> Result<()> {
    ...
    let out = span_stdout_with_stdin(&repo, &["list", "--batch", "--porcelain"], "file1.txt\n")?;
...
#[test]
fn ls_batch_porcelain_emits_no_spans_for_miss() -> Result<()> {
    let repo = TestRepo::seeded()?;
    commit_span(&repo, "alpha", "file1.txt#L1-L5", "alpha why")?;

    let out = span_stdout_with_stdin(&repo, &["list", "--batch", "--porcelain"], "missing.txt\n")?;

    assert_eq!(out.trim(), "no spans");
    Ok(())
}
...
fn ls_batch_porcelain_handles_multiple_queries() -> Result<()> {
    ...
    let out = span_stdout_with_stdin(
        &repo,
    ...
    assert_eq!(
        lines,
        vec!["alpha\tfile1.txt\t1-5", "no spans", "beta\tfile2.txt\t1-3"]
    );
    Ok(())
}

#[test]
fn ls_batch_porcelain_includes_staged_only_spans_across_queries() -> Result<()> {
    let repo = TestRepo::seeded()?;
    repo.span_stdout(["add", "pending-one", "file1.txt#L1-L2"])?;
    repo.span_stdout(["add", "pending-two", "file2.txt#L1-L3"])?;
...
#[test]
fn ls_batch_porcelain_includes_staged_adds_on_committed_spans() -> Result<()> {
    let repo = TestRepo::seeded()?;
    commit_span(&repo, "alpha", "file1.txt#L1-L5", "alpha why")?;
    repo.span_stdout(["add", "alpha", "file2.txt#L1-L3"])?;
...
fn ls_search_matches_name() -> Result<()> {
    let repo = TestRepo::seeded()?;
    commit_span(&repo, "alpha", "file1.txt#L1-L5", "some why")?;
    commit_span(&repo, "beta", "file2.txt#L1-L3", "other why")?;
    let out = repo.span_stdout(["list", "--search", "alpha"])?;
...
fn ls_search_matches_why_line() -> Result<()> {
    ...
    commit_span(&repo, "beta", "file2.txt#L1-L3", "unrelated relationship")?;
    let out = repo.span_stdout(["list", "--search", "parser"])?;
    assert!(
        out.contains("## alpha"),
...
fn ls_search_matches_anchor_address() -> Result<()> {
    ...
    commit_span(&repo, "alpha", "file1.txt#L1-L5", "alpha why")?;
    commit_span(&repo, "beta", "file2.txt#L1-L3", "beta why")?;
    let out = repo.span_stdout(["list", "--search", "file2"])?;
...
fn ls_search_case_sensitive_with_flag() -> Result<()> {
    ...
    let out = repo.span_stdout(["list", "--search", "(?-i)parser"])?;
    assert_eq!(
        out.trim(),
        "No spans match the filters.",
...
fn ls_search_invalid_regex_exits_two() -> Result<()> {
    let repo = TestRepo::seeded()?;
    let out = repo.run_span(["list", "--search", "[invalid"])?;
    assert_eq!(
        out.status.code(),
        Some(1),
