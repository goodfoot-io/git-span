//! CLI: `git span drift --fix` — re-anchor drifted Moved/Changed records
...
fn fully_fresh_span_is_absent_from_scan() -> Result<()> {
    ...
    let stdout = repo.span_stdout(["drift"])?;
...
fn fix_reanchors_whitespace_only_changed_at_head_layer() -> Result<()> {
    ...
    assert!(
        !span.contains(&format!("file1.txt#L1-L5 rk64:{original}")),
        "HEAD-layer whitespace-only change must not keep the drifted original hash; got:\n{span}"
    );
    assert!(
...
fn fix_reanchors_whitespace_only_changed_several_commits_back_at_head_layer() -> Result<()> {
    ...
    assert!(
        !span.contains(&format!("file1.txt#L1-L5 rk64:{original}")),
        "multi-commit-back whitespace-only change must not keep the drifted original hash; got:\n{span}"
    );
    assert!(
        span.contains(&format!("file1.txt#L1-L5 rk64:{reanchored}")),
...
fn fix_leaves_changed_anchor_when_history_walk_is_exhausted() -> Result<()> {
    ...
    let out2 = repo.run_span(["drift", "--no-exit-code"])?;
    let stdout = String::from_utf8_lossy(&out2.stdout);
    assert!(
        stdout.contains("## m"),
        "anchor must still surface as drifted after an exhausted walk; stdout=\n{stdout}"
    );
    Ok(())
}
...
fn fix_rejected_with_json_format() -> Result<()> {
    let repo = TestRepo::seeded()?;
    seed_span(&repo, "m", "file1.txt#L1-L5", "why")?;
    let out = repo.run_span(["drift", "--fix", "--format", "json"])?;
...
fn json_porcelain_unchanged_for_drifted_input() -> Result<()> {
    ...
    let out = repo.run_span(["drift", "--format", "json", "--no-exit-code"])?;
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
    ...
    // Porcelain
    let out = repo.run_span(["drift", "--format", "porcelain", "--no-exit-code"])?;
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
fn fix_renamed_orphan_pending_commit_reports_not_deleted() -> Result<()> {
    ...
    //
    // The single `drift --fix` invocation both resolves the conflict AND
...
"
    ...
    assert!(
        stdout.contains("resolved, pending commit") || stdout.contains("0 drift"),
...
fn fix_moved_with_worktree_shifts_reanchors_against_head() -> Result<()> {
    ...
    // Sanity: the span reports MOVED (exit code 1 is expected when drift).
    let drift_out = repo.run_span(["drift"])?;
    let drift = String::from_utf8_lossy(&drift_out.stdout);
    assert!(
        drift.contains("moved to"),
        "drift must report MOVED; got:\n{drift}"
    );
    ...
    // The drift output reports ResolvedPendingCommit: the span file is
    ...
    let drift_after_out = repo.run_span(["drift", "--no-exit-code"])?;
    let drift_after = String::from_utf8_lossy(&drift_after_out.stdout);
    assert!(
        drift_after.contains("resolved, pending commit") || drift_after.contains("0 drift"),
        "span must be clean after fix; got:\n{drift_after}"
    );
    assert_eq!(
        drift_after_out.status.code(),
        Some(0),
