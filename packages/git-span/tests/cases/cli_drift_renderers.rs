...
fn discovery_clean_head_pinned_span_uses_fast_path() -> Result<()> {
    ...
    assert!(
        !stderr.contains("git-span perf: resolver.resolve-drift-spans"),
...
fn discovery_porcelain_filters_clean_spans_before_rendering() -> Result<()> {
    ...
    let out = repo.run_span(["drift", "--format=porcelain"])?;
    assert_eq!(out.status.code(), Some(1));
...
fn porcelain_layered_includes_src_column() -> Result<()> {
    ...
    seed(&repo, "m")?;
    drift_in_head(&repo)?;
    let out = repo.run_span(["drift", "m", "--format=porcelain"])?;
    let text = String::from_utf8_lossy(&out.stdout);
    let line = text
        .lines()
...
fn human_pending_ops_render_range_addresses() -> Result<()> {
    ...
    let out = repo.span_stdout(["drift", "m", "--no-exit-code"])?;
...
#[test]
fn named_drift_clean_new_span_reports_zero_drift() -> Result<()> {
    ...
    // lookup renders no block and prints the 0-drift summary instead.
    let repo = TestRepo::seeded()?;
    repo.span_stdout(["add", "new-span", "file1.txt#L1-L5"])?;
    let out = repo.span_stdout(["drift", "new-span", "--no-exit-code"])?;
    assert!(
        !out.contains("## new-span"),
        "clean named span must not render a block; stdout={out}"
    );
    assert!(out.contains("0 drift across"), "stdout={out}");
    Ok(())
}
...
fn human_fresh_sibling_row_has_no_trailing_parenthesis() -> Result<()> {
    // New: unified block shows all anchors. Drifted get suffix, fresh appear bare.
    let repo = TestRepo::seeded()?;
    ...
    let out = repo.run_span(["drift", "m"])?;
    let text = String::from_utf8_lossy(&out.stdout);
    // Drifted anchor carries a suffix; fresh appears bare.
    assert!(
        text.contains("file1.txt#L1-L5 — changed"),
        "drifted anchor should carry status suffix, got: {text}"
    );
    assert!(
...
fn missing_file_arg_exits_one_with_diagnostic() -> Result<()> {
    let repo = TestRepo::seeded()?;
    let out = repo.run_span(["drift", "nonexistent-file"])?;
    assert_eq!(out.status.code(), Some(1));
...
fn existing_file_with_no_span_does_not_error() -> Result<()> {
    let repo = TestRepo::seeded()?;
    // file2.txt exists in the seeded worktree but no span tracks it. Drift
    // should exit 0 silently — "no span involves this file" is not an error.
    let out = repo.run_span(["drift", "file2.txt"])?;
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn deleted_span_root_file_arg_does_not_error() -> Result<()> {
    ...
    // this exercises the same "nothing to scan here" treatment drift already
    // gives an existing, unanchored file — regardless of whether the
    // span-root path still exists on disk.
    let out = repo.run_span(["drift", ".span/m"])?;
    assert_eq!(
        out.status.code(),
        Some(0),
...
fn file_path_arg_resolves_span_via_path_index() -> Result<()> {
    ...
    let out = repo.run_span(["drift", "file1.txt", "--format=json"])?;
    assert_eq!(
        out.status.code(),
        Some(1),
        "drifted span should exit 1, stderr: {}",
        String::from_utf8_lossy(&out.stderr)
    );
    let v: Value = serde_json::from_slice(&out.stdout).expect("valid json");
    assert_eq!(
...
fn mixed_span_name_and_path_args_are_deduplicated() -> Result<()> {
    ...
    let out = repo.run_span(["drift", "m", "file1.txt", "--format=json"])?;
    assert_eq!(
        out.status.code(),
        Some(1),
        "drifted span should exit 1, stderr: {}",
...
fn all_args_resolve_to_clean_span_exits_zero() -> Result<()> {
    ...
    // Span is clean (no drift). Drift by span name should exit 0.
    let out = repo.run_span(["drift", "clean-span"])?;
    assert_eq!(out.status.code(), Some(0));
    Ok(())
}
...
// partitions the run's drifted spans into connected components by shared
// anchored file.
// ---------------------------------------------------------------------------
...
/// halves of `file1.txt`, so a span seeded here is expected to drift too.
...
/// drift together; `o` (`file2.txt#L1-L5`) is unrelated and drifts on
...
fn human_cluster_lists_shared_spans_and_independent_singleton() -> Result<()> {
    ...
    let out = repo.span_stdout(["drift", "--cluster", "--no-exit-code"])?;
...
fn json_cluster_field_lists_spans_and_shared_files() -> Result<()> {
    ...
    let out = repo.span_stdout(["drift", "--cluster", "--format=json", "--no-exit-code"])?;
...
fn porcelain_cluster_comment_lines_list_spans_and_shared_files() -> Result<()> {
    ...
    let out = repo.span_stdout(["drift", "--cluster", "--format=porcelain", "--no-exit-code"])?;
    assert!(
...
fn cluster_output_is_identical_on_warm_cache_hit() -> Result<()> {
    ...
    // `None` on every warm cache_v2 hit (the fast path `drift` is optimized
    ...
    let first = repo.span_stdout(["drift", "--cluster", "--format=json", "--no-exit-code"])?;
    let second = repo.span_stdout(["drift", "--cluster", "--format=json", "--no-exit-code"])?;
    assert_eq!(
        first, second,
...
fn porcelain_cluster_escapes_file_paths_with_commas() -> Result<()> {
    ...
    // Both m and n should be detected as drifted and clustered together via their shared file.
    assert!(
