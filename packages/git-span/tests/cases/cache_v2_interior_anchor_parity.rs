...
/// Seed two spans: a normal partially-drifted span over source files, and a
/// second span whose anchor points INSIDE `.span/` (an interior anchor).
fn seed_interior_anchor_corpus(repo: &TestRepo) -> Result<()> {
    ...
    // Drift one anchored region of the normal span via a COMMIT (clean tree).
    repo.write_file("a.txt", "A1\nA2\nA3\na4\na5\n")?;
