//! CLI: `git span drift --fix` mid-merge — both drifted anchors on the same
...
//! record lookup silently misses (drift_fix.rs L664-L674), leaving the
...
//! 6. Run `git span drift --fix` and check the `.span` file on disk.
...
";
    ...
    // Old drifted extents must be gone.
    assert!(
    ...
    // ---- verify no drifted anchors on re-run ----
    // After the first --fix corrected both anchors, a second --fix should
    // find zero drifted anchors. Coalescing may normalize adjacent Fresh
    // line ranges into a single wider range, so the span file may not be
    // byte-identical — but it must not still be drifted.
    let out2 = repo.run_span(["drift", "--fix", "--no-exit-code"])?;
    let stdout2 = String::from_utf8_lossy(&out2.stdout);
    let span_after_rerun = read_span(&repo, "parse/pair")?;
    assert!(
        !span_after_rerun.contains("f.js#L4-L6 "),
        "drift L4-L6 must be gone after re-run;\nspan:\n{span_after_rerun}\nstdout:\n{stdout2}",
    );
    assert!(
    ...
    let out3 = repo.run_span(["drift", "--fix"])?;
    let stdout3 = String::from_utf8_lossy(&out3.stdout);
    assert!(
        stdout3.contains("0 drift"),
        "post-merge must report 0 drift;\nstdout:\n{stdout3}",
    );
