//! Acceptance tests for `git span tree`.
//!
//! These are the executable specification for the subcommand. Expected values
//! are derived from the authoritative prototype `scripts/git-span-tree-demo.mjs`,
...
/// Add a span via the CLI and commit it.
fn add_span(repo: &TestRepo, slug: &str, paths: &[&str]) -> Result<()> {
...
fn clique_grouping_fully_connected_renders_one_line() -> Result<()> {
    ...
    // span-abcd connects all four, making them a single maximal clique
    add_span(&repo, "span-abcd", &["a.rs", "b.rs", "c.rs", "d.rs"])?;

    let out = repo.span_stdout(["tree", "a.rs"])?;
...
fn chain_topology_yields_two_overlapping_cliques() -> Result<()> {
    ...
    add_span(&repo, "span-xa", &["x.rs", "a.rs"])?;
    add_span(&repo, "span-xb", &["x.rs", "b.rs"])?;
    add_span(&repo, "span-xc", &["x.rs", "c.rs"])?;
    add_span(&repo, "span-ab", &["a.rs", "b.rs"])?;
    add_span(&repo, "span-bc", &["b.rs", "c.rs"])?;

    let out = repo.span_stdout(["tree", "x.rs"])?;
...
fn clique_as_unit_expansion_no_mutual_relisting() -> Result<()> {
    ...
    add_span(&repo, "span-abcd", &["a.rs", "b.rs", "c.rs", "d.rs"])?;
    add_span(&repo, "span-ae", &["a.rs", "e.rs"])?;
...
fn loop_guard_prevents_ancestor_reexpansion() -> Result<()> {
    ...
    add_span(&repo, "span-ab", &["a.rs", "b.rs"])?;
    add_span(&repo, "span-bc", &["b.rs", "c.rs"])?;
    add_span(&repo, "span-ca", &["c.rs", "a.rs"])?;
...
fn depth_zero_yields_roots_only() -> Result<()> {
    ...
    let out = repo.span_stdout(["tree", "--depth", "0", "a.rs"])?;
    let non_empty: Vec<&str> = out.lines().filter(|l| !l.trim().is_empty()).collect();
    assert_eq!(
        non_empty.len(),
        1,
...
fn default_depth_three_truncates_deeper_chains() -> Result<()> {
    ...
    add_span(&repo, "span-ab", &["a.rs", "b.rs"])?;
    add_span(&repo, "span-bc", &["b.rs", "c.rs"])?;
    add_span(&repo, "span-cd", &["c.rs", "d.rs"])?;
    add_span(&repo, "span-de", &["d.rs", "e.rs"])?;
...
fn explicit_depth_flag_respected() -> Result<()> {
    ...
    let out1 = repo.span_stdout(["tree", "--depth", "1", "a.rs"])?;
    assert!(
        !out1.contains("c.rs"),
    ...
    let out4 = repo.span_stdout(["tree", "--depth", "4", "a.rs"])?;
    assert!(
        out4.contains("e.rs"),
...
fn glob_and_exact_path_produce_identical_output() -> Result<()> {
    ...
    let out_exact = repo.span_stdout(["tree", "a.rs"])?;
    let out_glob = repo.span_stdout(["tree", "a.*"])?;
    assert_eq!(
        out_exact, out_glob,
...
fn glob_resolution_is_repo_relative_not_cwd_relative() -> Result<()> {
    ...
    add_span(&repo, "span-ab", &["src/a.rs", "src/b.rs"])?;

    // Exact repo-relative path must work from the repo root
    let out = repo.span_stdout(["tree", "src/a.rs"])?;
    assert!(
...
fn unknown_arg_exits_nonzero() -> Result<()> {
    ...
    add_span(&repo, "span-a", &["a.rs"])?;

    let out = repo.run_span(["tree", "nonexistent.rs"])?;
    assert!(
        !out.status.success(),
...
/// A bare `git span tree` with zero positional args must fail closed
...
fn no_args_exits_nonzero() -> Result<()> {
    ...
    let out = repo.run_span(["tree"])?;
    assert!(
        !out.status.success(),
        "bare `git span tree` (no args) must exit non-zero:\nstdout: {}\nstderr: {}",
...
fn mixed_case_siblings_use_deterministic_byte_order() -> Result<()> {
    ...
    add_span(&repo, "span-rz", &["root.rs", "Z.rs"])?;
    add_span(&repo, "span-rb", &["root.rs", "b.rs"])?;

    let out = repo.span_stdout(["tree", "root.rs"])?;
...
fn json_format_top_level_is_array() -> Result<()> {
    ...
    let out = repo.run_span(["tree", "--format", "json", "a.rs"])?;
    assert!(
        out.status.success(),
...
fn output_is_deterministic_across_runs() -> Result<()> {
    ...
    add_span(&repo, "span-bc", &["b.rs", "c.rs"])?;
    add_span(&repo, "span-cd", &["c.rs", "d.rs"])?;
    add_span(&repo, "span-ac", &["a.rs", "c.rs"])?;

    let run1 = repo.span_stdout(["tree", "a.rs"])?;
    let run2 = repo.span_stdout(["tree", "a.rs"])?;
    let run3 = repo.span_stdout(["tree", "a.rs"])?;
...
fn deterministic_ordering_for_multiple_roots() -> Result<()> {
    ...
    let out1 = repo.span_stdout(["tree", "a.rs", "c.rs"])?;
