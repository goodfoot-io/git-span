...
/// `git span drift m` exits 0 and prints "resolved, pending commit" in
/// human output.
#[test]
fn drift_command_exits_zero_with_resolved_pending_commit() -> Result<()> {
    let repo = TestRepo::seeded()?;
    reanchor_unstaged_source(&repo)?;
...
/// Repeated `git span drift m` without state change is stable (no flip
