...
fn generated_manpage_matches_checked_in_artifact() {
    ...
    // The generated-artifact banner must open the file; the byte-equality
    // assertion below then transitively enforces it on the checked-in page.
    let marker = b".\\\" GENERATED by gen-manpage. Do not edit by hand.";
    assert!(
        generated.starts_with(marker),
        "generated git-span.1 does not open with the GENERATED-by banner; the generator must stamp it"
    );
