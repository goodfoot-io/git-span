//! CLI: `git span drift` — default human output (§10.4).
...
/// `git span drift` detects and `--patch`/`--stat` diff against the
...
fn workspace_scan_all_clean_exit_zero() -> Result<()> {
    ...
    assert!(
        ...
        "summary must mention 0 drift when all spans are clean, got: {stdout}"
    );
...
/// A drifted span surfaced by a no-argument scan must list its *complete*
...
#[test]
fn workspace_scan_drift_span_shows_fresh_sibling_and_why() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    let _warm = repo.run_span(["drift"])?;
    let out = repo.run_span(["drift"])?; // no args → workspace scan (warm)
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
#[test]
fn drift_recursive_glob_matches_nested_anchored_path() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    let stdout = repo.span_stdout(["drift", "wiki/**/*", "--no-exit-code"])?;
    assert!(
...
#[test]
fn drift_single_star_glob_matches_single_segment_anchored_path() -> Result<()> {
    let repo = TestRepo::seeded()?;
