...
fn read_mode_flags_select_layers() -> Result<()> {
    ...
    for flag in ["--head", "--staged", "--worktree"] {
        let out = repo.span_stdout(["stale", "m", flag, "--no-exit-code"])?;
        assert!(
    ...
    let head_only = repo2.span_stdout(["stale", "m", "--head", "--no-exit-code"])?;
    assert!(
        !head_only.contains("changed"),
        "--head must ignore a staged-only change; out=\n{head_only}"
    );
    for flag in ["--staged", "--worktree"] {
        let out = repo2.span_stdout(["stale", "m", flag, "--no-exit-code"])?;
        assert!(
            out.contains("changed"),
    ...
    for flag in ["--head", "--staged"] {
        let out = repo3.span_stdout(["stale", "m", flag, "--no-exit-code"])?;
        assert!(
            !out.contains("changed"),
            "{flag} must ignore a worktree-only change; out=\n{out}"
        );
    }
    let wt = repo3.span_stdout(["stale", "m", "--worktree", "--no-exit-code"])?;
    assert!(
        wt.contains("changed"),
