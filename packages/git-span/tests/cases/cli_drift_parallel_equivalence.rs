...
//!
//! [`cli_drift_fix_perf_equivalence.rs`](tests/cases/cli_drift_fix_perf_equivalence.rs)
...
//! [`COLD_DRIFT_MIN_ANCHORS_PER_TASK`](src/resolver/engine/mod.rs) so rayon
...
/// A single `git span drift` invocation's captured output.
struct Run {
    stdout: Vec<u8>,
    stderr: Vec<u8>,
    code: Option<i32>,
...
/// Run `git span drift --no-exit-code --format <format>` in `repo`.
///
...
fn run_drift(repo: &TestRepo, format: &str, threads: Option<&str>, cache_off: bool) -> Result<Run> {
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_git-span"));
    cmd.current_dir(repo.path());
    cmd.args(["drift", "--no-exit-code", "--format", format]);
...
/// Run `git span drift` (default `human`) capturing its *error-path* outcome,
/// with an optional `RAYON_NUM_THREADS`. No `--no-exit-code`: a hard resolution
/// error must surface its non-zero exit code.
fn run_drift_erroring(repo: &TestRepo, threads: Option<&str>) -> Result<Run> {
    let mut cmd = Command::new(env!("CARGO_BIN_EXE_git-span"));
    cmd.current_dir(repo.path());
    cmd.args(["drift"]);
    if let Some(t) = threads {
...
///
/// Four 60-line files, 30 anchors each (120 total, > `COLD_DRIFT_MIN_ANCHORS_PER_TASK`):
...
fn dirty_tree_cache_off_matches_warm_across_formats() -> Result<()> {
    ...
    for format in ["human", "porcelain", "json"] {
        ...
        clear_store(&repo)?;
        let _ = run_drift(&repo, format, None, false)?;
        let warm = run_drift(&repo, format, None, false)?;
        let cache_off = run_drift(&repo, format, None, true)?;
...
/// push the batch above `COLD_DRIFT_MIN_ANCHORS_PER_TASK` so rayon forks for real.
...
fn error_path_surfaces_first_input_order_error_identically_across_threads() -> Result<()> {
    ...
    for iter in 0..24 {
        ...
        clear_store(&repo)?;
        let run = run_drift_erroring(&repo, threads)?;
...
fn stress_shared_contention_no_divergence_over_iterations() -> Result<()> {
    ...
    let ground: Vec<(&str, Vec<u8>, Option<i32>)> = ["human", "porcelain", "json"]
        .into_iter()
        .map(|f| -> Result<_> {
            let r = run_drift(&repo, f, None, true)?;
    ...
    for iter in 0..STRESS_ITERS {
        for (format, ground_stdout, ground_code) in &ground {
            ...
            clear_store(&repo)?;
            let run = run_drift(&repo, format, None, false)?;
            assert_eq!(
                as_str(&run.stdout),
