//! Regression: `git span add` must refuse an anchor whose target path is
//! gitignored.
//!
...
fn add_rejects_gitignored_anchor_target() -> Result<()> {
    ...
    let out = repo.run_span(["add", "ignored-demo", "generated.ts#L1-L5"])?;
    assert!(
        !out.status.success(),
    ...
    // The reject is fail-closed at the source: no span file is written.
    assert!(
        !repo.path().join(".span/ignored-demo").exists(),
        "a rejected add must not create the span file"
    );
    Ok(())
}
...
fn add_allows_untracked_not_ignored_anchor_target() -> Result<()> {
    ...
    let out = repo.run_span(["add", "transient-demo", "newcode.ts#L1-L3"])?;
    assert!(
        out.status.success(),
    ...
    assert!(repo.path().join(".span/transient-demo").exists());
    Ok(())
}
...
fn add_allows_force_added_tracked_path_matching_ignore_pattern() -> Result<()> {
    ...
    let out = repo.run_span(["add", "forced-demo", "generated.ts#L1-L5"])?;
    assert!(
        out.status.success(),
