//! CLI: positional `git span drift <path>` visibility for clean spans.
//!
//! Every `git span drift` invocation is a drift report, scoped or not. A
//! no-argument workspace scan omits fully-clean spans, and a scoped query
//! (a path, glob, or an explicit span name) does the same: only drifted spans
//! render, in every format. A scoped query that finds nothing drift prints a
//! "0 drift across N spans (A anchors checked)" summary so the operator gets
...
fn positional_human_drops_clean_span() -> Result<()> {
    ...
    seed_fixture(&repo)?;
    drift(&repo)?;
    let out = repo.run_span(["drift", "file1.txt"])?;
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
        stdout.contains("drifted-span"),
...
fn positional_porcelain_drops_clean_span() -> Result<()> {
    ...
    seed_fixture(&repo)?;
    drift(&repo)?;
    let out = repo.run_span(["drift", "file1.txt", "--format=porcelain"])?;
    assert_eq!(out.status.code(), Some(1), "drift present → exit 1");
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
/// A direct span-name request is a drift report too: `drift clean-span`
