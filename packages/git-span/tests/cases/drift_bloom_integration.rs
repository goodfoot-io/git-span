...
//!
//! 1. `drift_spans_succeeds_without_commit_graph` — the changed-path
//!    Bloom filter is a walk accelerator, not a correctness gate. An
//!    ordinary repo (no gc, no opt-in `core.commitGraph`) has no
//!    commit-graph; `drift` must still resolve, never surfacing a
...
use anyhow::Result;
use git_span::types::{EngineOptions, LayerSet};
...
fn drift_spans_succeeds_without_commit_graph() -> Result<()> {
    ...
    // Create a file and a span. We must have at least one span so that
    // drift_spans attempts to build the reverse-indexed walk (the Bloom
    // open is unconditional in build_reverse_walk even for empty span lists,
    // but a single-span path exercises more of the pipeline).
    ...
    let result = git_span::drift_spans(
        &gix,
        ".span",
        EngineOptions {
            layers: LayerSet {
    ...
    let resolved = result.expect("drift_spans must succeed without a commit-graph");
    // The single committed span's anchor is fresh, so it is not
    // reportable; the call simply returns without error.
    assert!(
        resolved.iter().all(|m| m
            .anchors
            .iter()
            .all(|a| a.status == git_span::types::AnchorStatus::Fresh)),
...
fn bloom_false_positive_counter_is_zero_for_true_positives() -> Result<()> {
    ...
    // Run `git span drift` with perf counters enabled.
    ...
    assert_eq!(
        out.status.code(),
        Some(1),
        "both anchors drifted; drift must exit 1. stdout=\n{stdout}\nstderr=\n{stderr}"
    );
    assert!(
