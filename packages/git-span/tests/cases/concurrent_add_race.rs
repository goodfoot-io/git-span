...
//! concurrent `git span add` invocations on the same span silently lose
//! anchors.
//!
...
//! against the same span name, each adding a different anchor, read the
//! same starting span content, both mutate independently in memory, and
//! the slower write silently overwrites the faster.
//!
//! Test strategy:
//! 1. Seed a span with one anchor so it exists on disk.
//! 2. Launch two threads, each adding a different second anchor.
//! 3. After both threads complete, read the span back and assert all
...
//! `write_worktree_span` on the shared temp-file name (`.race.tmp`),
...
fn concurrent_add_race_loses_anchors() -> Result<()> {
    ...
    // `write_worktree_span` calls, one thread will fail with ENOENT.
