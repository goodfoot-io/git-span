...
//!
//! 1. `stale_spans_succeeds_without_commit_graph` — the changed-path
...
#[test]
fn stale_spans_succeeds_without_commit_graph() -> Result<()> {
    let repo = TestRepo::new()?;

    // Create a file and a span. We must have at least one span so that
    // stale_spans attempts to build the reverse-indexed walk (the Bloom
    ...
    let result = git_span::stale_spans(
        &gix,
        ".span",
        EngineOptions {
            layers: LayerSet {
    ...
    let resolved = result.expect("stale_spans must succeed without a commit-graph");
