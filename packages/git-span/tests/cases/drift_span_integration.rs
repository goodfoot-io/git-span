...
use git_span::{resolve_anchor, resolve_span, drift_spans};
...
fn commit_reanchor_replaces_moved_range_instead_of_adding_duplicate() -> Result<()> {
    ...
    // area): remove the drifted range, add the relocated one. The span
    ...
    let drift = repo.run_span(["drift", "m"])?;
    assert_eq!(drift.status.code(), Some(0));
    Ok(())
}
...
fn line_range_anchor_inside_submodule_promoted_directory_reports_submodule() -> Result<()> {
    ...
    assert_eq!(
        out.status.code(),
        Some(1),
        "SUBMODULE is a drift status → exit 1"
    );
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(
...
fn lfs_line_range_unchanged_worktree_reports_fresh() -> Result<()> {
    ...
    assert_eq!(
        out.status.code(),
        Some(0),
        "expected no drift findings; stdout=\n{stdout}\nstderr=\n{}",
...
/// Plan bullet: Rename-heavy changeset (>1000 paths): `drift` completes without
...
fn rename_heavy_changeset_completes_with_note() -> Result<()> {
    ...
    assert_eq!(
        out.status.code(),
        Some(1),
        "drift must report drift; stderr={}",
        String::from_utf8_lossy(&out.stderr)
    );
    assert!(
        !String::from_utf8_lossy(&out.stdout)
            .to_lowercase()
            .contains("orphan"),
        "the word 'orphaned' must never appear in drift output; stdout={}",
        String::from_utf8_lossy(&out.stdout)
    );
    let mr = resolve_span(&repo.gix_repo()?, ".span", "m", EngineOptions::full())?;
...
/// Plan bullet (coverage of `drift_spans`): worst-first ordering across spans.
#[test]
fn drift_spans_sorts_worst_first_smoke() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    repo.commit_all("mutate")?;
    let all = drift_spans(&repo.gix_repo()?, ".span", EngineOptions::full())?;
    assert!(
        all.iter()
...
/// A clean span (all anchors Fresh, no pending) must be excluded from `drift_spans`.
#[test]
fn drift_spans_excludes_clean_span() -> Result<()> {
    let repo = TestRepo::seeded()?;
    seed_line_range_span(&repo, "clean-only")?;
    let all = drift_spans(&repo.gix_repo()?, ".span", EngineOptions::full())?;
    assert!(
        !all.iter().any(|m| m.name == "clean-only"),
        "clean span must not appear in drift_spans output"
    );
    Ok(())
}

/// A span with one Changed anchor must be included in `drift_spans`.
#[test]
fn drift_spans_includes_changed_span() -> Result<()> {
    let repo = TestRepo::seeded()?;
    ...
    assert!(
        ...
        "changed span must appear in drift_spans output"
    );
    Ok(())
}

/// When a repo has both a clean span and a drifted span, only the drifted span appears.
#[test]
fn drift_spans_filters_clean_leaves_drift() -> Result<()> {
    let repo = TestRepo::seeded()?;
...
/// Drift-present repo + `--format=json` → non-empty envelope, exit 1.
#[test]
fn drift_present_json_emits_envelope() -> Result<()> {
    let repo = TestRepo::seeded()?;
    seed_line_range_span(&repo, "drifty")?;
    repo.write_file(
        "file1.txt",
    ...
    assert!(
        !stdout.trim().is_empty(),
        "json output must not be empty when a span is drifted"
    );
    assert!(
