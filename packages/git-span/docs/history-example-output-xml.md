# `git span history` — canonical XML example output

This document is the reference specification for the XML renderer (`render_xml`).
Integration tests assert that `git span history billing/checkout-request-flow` produces
output structurally equivalent to the examples below.

## Scenario

A span `billing/checkout-request-flow` is created in commit `a1b2c3d`, modified in
...
- `<anchor>` on timeline entries carries `path` (combined git-span address) and `event`
...
- `<current>` block is emitted only when the working tree differs from HEAD.
  ...
  - Anchors added to the working tree (uncommitted `git span add`) carry
    `status="added in the working tree"` and their live content in CDATA.
  - Anchors removed from the working tree (uncommitted `git span remove`) carry
