//! Slice 8 performance gate: HEAD-only `drift` on a comparable fixture.
//!
...
fn build_fixture() -> Fixture {
    ...
    // Pin a span on each file using library API so renamed symbols fail at compile time.
    std::fs::create_dir_all(p.join(".span")).expect("create .span");
    for i in 0..10u32 {
...
fn bench_head_only(c: &mut Criterion) {
    let f = build_fixture();
    c.bench_function("drift_spans_head_only", |b| {
        b.iter(|| {
            let repo = gix::open(&f.repo_path).expect("open repo");
            let out = drift_spans(&repo, ".span", EngineOptions::committed_only()).expect("drift");
