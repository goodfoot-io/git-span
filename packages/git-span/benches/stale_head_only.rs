...
//! `stale_spans` on a small synthetic fixture seeded once per run
...
fn bench_head_only(c: &mut Criterion) {
    let f = build_fixture();
    c.bench_function("stale_spans_head_only", |b| {
        b.iter(|| {
            let repo = gix::open(&f.repo_path).expect("open repo");
            let out = stale_spans(&repo, ".span", EngineOptions::committed_only()).expect("stale");
