//! Phase 3 step 8: warm-run SLA benchmark for `git span drift`.
//!
//! Two scenarios:
...
fn bench_cold(c: &mut Criterion) {
    ...
    g.bench_function("cold", |b| {
        b.iter(|| {
            ...
            let out = drift_spans(&repo, ".span", EngineOptions::full()).expect("drift");
...
fn bench_warm(c: &mut Criterion) {
    ...
    // Prime the cache with one cold run before measuring.
    {
        clear_cache(&f.repo_path);
        let repo = gix::open(&f.repo_path).expect("open repo");
        drift_spans(&repo, ".span", EngineOptions::full()).expect("prime");
    }
    ...
    // ---------------------------------------------------------------------------
    {
        ...
        if median_ms > SLA_WARM_MS {
            panic!(
                "[drift_warm] warm-clean SLA breach: median {median_ms:.1} ms > {SLA_WARM_MS} ms \
