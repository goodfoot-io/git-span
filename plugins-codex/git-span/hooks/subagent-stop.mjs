...
// src/codex/subagent-stop.ts
function createHandler(layout = DEFAULT_SESSION_LAYOUT) {
  return async (input, ctx) => {
    try {
      createSnapshotStore(ctx.logger, void 0, layout).removeSession(input.session_id, input.agent_id);
      return void 0;
    } catch (err) {
...
var subagent_stop_default = subagentStopHook({ timeout: 1e4 }, createHandler());

// src/codex/subagent-stop-entry.ts
