...
function renderLeafLines(node, ownPrefix, childPrefix, groupMax) {
  const { name } = node;
  ...
  return sorted.map((entry, i) => {
    ...
    if (label === null) {
      const base2 = i === 0 ? `${ownPrefix}${name}` : `${childPrefix}${blank}`;
      return `${base2}${entry.suffix}`;
    }
