...
function isFileOnDisk(absPath) {
  try {
    return fs2.statSync(absPath).isFile();
  } catch {
    return false;
  }
}
function contentMatches(post, filePath) {
  try {
    if ("exact" in post) return fs2.readFileSync(filePath, "utf8") === post.exact;
    if ("suffix" in post) {
      const content = fs2.readFileSync(filePath, "utf8");
      return content.endsWith(post.suffix) || content.endsWith(`${post.suffix}
`);
    }
    if ("empty" in post) return fs2.statSync(filePath).size === 0;
    return fs2.statSync(filePath).size === post.size;
  } catch {
    return false;
  }
}
...
function evaluateWriteGate(input, probeCache) {
  ...
  if (input.sourcePath !== void 0) {
    if (fileExists(input.sourcePath)) {
      ...
      let dst;
      try {
        src = fs2.readFileSync(input.sourcePath, "utf8");
        dst = fs2.readFileSync(input.filePath, "utf8");
      } catch {
        return "decisiveFail";
      }
...
function shouldSurfaceRecordlessNote(sessionId, logger2, layout) {
  try {
    mkdirSync5(layout.dir(sessionId), { recursive: true, mode: 448 });
    writeFileSync3(layout.recordlessNoteFile(sessionId), "", { flag: "wx" });
    return true;
  } catch (err) {
...
function loadAdvisorIgnore(repoRoot) {
  try {
    const content = fs4.readFileSync(nodePath3.join(repoRoot, ADVISOR_IGNORE_REL), "utf8");
...
function probeGitSpanVersion(repoRoot, timeoutMs) {
  try {
    const out = execFileSync5("git", ["span", "--version"], {
      cwd: repoRoot,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "pipe"],
      timeout: timeoutMs,
...
function createDefaultAdvisorExecutors(timeoutMs = DEFAULT_TIMEOUT_MS2) {
  return {
    fix: async (paths, cwd) => {
      ...
      if (!repoRoot || paths.length === 0) return;
      try {
        execFileSync5("git", ["span", "drift", ...paths, "--fix"], {
          cwd: repoRoot,
          encoding: "utf8",
          stdio: ["ignore", "pipe", "pipe"],
          timeout: timeoutMs,
    ...
    drift: async (paths, cwd) => {
      ...
      let out;
      try {
        out = execFileSync5("git", ["span", "drift", "--format", "porcelain", ...paths], {
          cwd: repoRoot,
          encoding: "utf8",
          stdio: ["ignore", "pipe", "pipe"],
          timeout: timeoutMs,
    ...
    list: async (paths, cwd) => {
      ...
      let out;
      try {
        out = execFileSync5("git", ["span", "list", "--porcelain", ...paths], {
          cwd: repoRoot,
          encoding: "utf8",
          stdio: ["ignore", "pipe", "pipe"],
          timeout: timeoutMs,
    ...
    listBlocks: async (names, cwd) => {
      ...
      try {
        return execFileSync5("git", ["span", "list", ...names], {
          cwd: repoRoot,
          encoding: "utf8",
          stdio: ["ignore", "pipe", "pipe"],
          timeout: timeoutMs,
...
function countFileLines(absolutePath) {
  try {
    if (!statSync5(absolutePath).isFile()) return null;
    const content = readFileSync7(absolutePath, "utf8");
...
function countGitBlobLines(cwd, rev, path) {
  try {
    const out = execFileSync7("git", ["show", `${rev}:${path}`], {
      cwd,
      encoding: "utf8",
      stdio: ["ignore", "pipe", "ignore"]
    });
...
function isExistingDirectory(absolutePath) {
  try {
    return statSync6(absolutePath).isDirectory();
  } catch {
    return false;
  }
}
...
function readPatchFile(absolutePath) {
  try {
    return readFileSync8(absolutePath, "utf8");
  } catch {
    return null;
  }
}
...
function createHandler3(layout = DEFAULT_SESSION_LAYOUT) {
  return async (input, ctx) => {
    try {
      ...
      if (command === null) return void 0;
      const effectiveCwd = workdir !== null && !/[`$]/.test(workdir) ? resolvePath4(input.cwd ?? "", workdir) : input.cwd ?? "";
...
var snapshot_default = preToolUseHook(
  { matcher: "Bash|shell|exec|local_shell|exec_command", timeout: 1e4 },
  createHandler3()
);

// src/codex/snapshot-entry.ts
