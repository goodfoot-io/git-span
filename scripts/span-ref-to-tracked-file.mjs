#!/usr/bin/env node
// Migrate git-span from the legacy ref-backed catalog to the tracked-file
// layout described in card main-78: one UTF-8 text file per span under the
// span root (default `.span`).
//
...
//   --span-dir     Override the span root. Precedence (main-78):
//                  --span-dir > GIT_SPAN_DIR > `git config git-span.dir` >
//                  `.span`.
//   --prune-refs   After writing files, delete the legacy ref namespaces
//                  (refs/spans/v1/*, refs/anchors/v1/*,
//                  refs/spans-index/v1/*).
...
function resolveSpanRoot() {
  ...
  if (norm === '..' || norm.startsWith('../') || norm.split('/').includes('..')) {
    throw new Error(`span root must not contain "..": ${candidate}`);
  }
  if (norm === '.git' || norm.startsWith('.git/')) {
    throw new Error(`span root must not live inside .git: ${candidate}`);
  }
  return norm;
}
const spanRoot = resolveSpanRoot();

// --- read + decode the legacy catalog ---------------------------------------
function catalogSpanBlobs() {
  const commit = gitTry(['for-each-ref', '--format=%(objectname)', 'refs/spans/v1/catalog']);
  if (!commit) return [];
  const tree = git(['ls-tree', 'refs/spans/v1/catalog^{tree}']);
  const out = [];
  for (const line of tree.split('\n')) {
    ...
    if (!entry.endsWith('.span')) continue;
    const name = entry.slice(0, -'.span'.length).split('++').join('/');
    out.push({ name, oid });
  }
...
for (const { name, oid } of blobs) {
  ...
  if (!span.anchors_v2.length) {
    console.warn(`[${name}] no anchors; skipping`);
    continue;
  }
  const relPath = join(spanRoot, name);
  const absPath = join(repoRoot, relPath);
  const content = renderSpanFile(span);
  ...
  if (dryRun) {
    ...
    console.log(`wrote ${relPath} (${span.anchors_v2.length} anchor(s))`);
  }
  written++;
}
...
if (values['prune-refs']) {
  const namespaces = ['refs/spans/v1/', 'refs/anchors/v1/', 'refs/spans-index/v1/'];
  const refs = [];
