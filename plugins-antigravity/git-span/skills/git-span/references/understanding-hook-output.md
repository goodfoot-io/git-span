...
- **Touch hook** (`PostToolUse`, joined to a `PreToolUse` stash on matcher
  `run_command`) — Antigravity's `PostToolUse` input carries only a step index
  (no tool name, arguments, or response), so a `PreToolUse` handler stashes
  each `run_command` call on disk and the `PostToolUse` handler joins on that
  step index to attribute it, gating on the absence of the host's `error`
  field. It heals positional drift silently right after the command completes;
  the `<git-span>` blocks it renders are stashed and delivered after the
  invocation's tool batch as one ephemeral message, since `PostToolUse` has no
  reply channel.
- **Advisor** (`PreToolUse`, matcher `run_command`) — fires before `git
  commit`/`git push`/`git status` runs. For `git commit`/`git push`, it holds
  the command once when the resolved changeset carries real span debt, so the
  report gets read; it never enforces, and an identical retry proceeds. A hold
  is a `deny` decision whose reason — the full checklist — reaches the model
  as the denial message (live-verified; of the four platforms this is the one
  whose hold is confirmed to stop the command). A plain `git status` is never
  held — report-only advisories have no allow-with-message channel on
  `PreToolUse`, so its checklist is stashed and delivered after the
  invocation's tool batch as an ephemeral message, re-reported live every
  call, and the check never touches the consider-once memo a real `git
  commit`/`git push` depends on.

One coverage gap is accepted for this version: Antigravity's dedicated
file-edit tool names are not pinned by its hook contract, so only
`run_command` shell activity is attributed — edits made through file tools
bypass touch attribution entirely (see `references/install-and-trust.md`).
The advisor is unaffected: it resolves the changeset from git state and sees
...
the touch pipeline renders a merged `<git-span>` block and delivers it after
the invocation's tool batch as one ephemeral message: a header line, one full
span section per surfaced span (sections separated by `---`), and a single
footer after a final `---`. A healthy span renders as:
...
- Any dedicated file read/edit/write tool call — unpinned tool names mean
  only `run_command` activity is attributed at all.
- A shell write to a path that doesn't yet exist on disk.
- A shell write that's a full-content replacement (no common prefix/suffix with
  what's on disk).
...
never a file read or edit. It resolves the actual changeset (staged files,
...
left. For `git commit`/`git push`, a hold becomes a `deny` decision whose
reason — one of two shapes below — reaches the model verbatim as the denial
message:
...
Dispatch a subagent with `invoke_subagent` to bring the coupled files back
into agreement (follow confirmed authority) — preserve anchor shape; if an
address changed, swap the old anchor for the new one with `git span replace`;
update or retire the why only if its meaning changed; require `git span drift
billing/checkout-request-flow` to report zero. Then retry. Load the
`reconcile` skill in the subagent. The hold will not fire again for the same
debt state. Conform a side only when confirmed authority or a satisfied gate
...
Dispatch a subagent with `invoke_subagent` to determine if this file carries
implicit dependencies and to then use `git span` to document them:
...
Load the `git-span` skill in the subagent.
</git-span>
```
...
above are stashed by the advisor and delivered after the invocation's tool
batch as an ephemeral message (never a `deny` — nothing is ever held), with
one difference: each drops its retry phrasing — drift drops `— then retry`
...
1. Semantic drift: dispatch a subagent with `invoke_subagent` to bring the
   coupled files back into agreement (loading the `reconcile` skill in the
   subagent), or just retry with the findings unchanged, since an identical
   set of findings is only held on once. Conform a side only when confirmed
   authority or a satisfied gate decides it; report ambiguity or an obsolete
   dependency.
2. Uncovered writes: dispatch a subagent with `invoke_subagent` to determine
   whether the uncovered files carry implicit dependencies and to use
   `git span` to document them (loading the `git-span` skill in the
   subagent), or just retry — the second attempt at an unchanged debt state
   passes.
...
ignored files, span metadata, and untracked files are dropped. The
`run_command` call's `Cwd` argument is used for both planning and post-tool
attribution, so an explicit working directory inside a different repository
is handled in that repository.
...
reuse a plan. Under Antigravity, `Stop` prunes the session's call-scoped
plans and stashes; the surfaced-span memo is deliberately left to its TTL
sweep (pruning it at a turn boundary would make identical spans re-inject
every turn), and opportunistic 30-day cleanup covers crashed sessions.

Antigravity's `PostToolUse` carries no tool response — success collapses to
the absence of the host's `error` field. A command whose input carries
`error` routes to failure attribution and can never surface a write (there is
no post-state evidence to verify against), and response-derived read
attribution is unavailable entirely — the degraded-parity caveat one notch
past OpenCode's. Interrupted calls stay silent.
...
`run_command` call's `Cwd` argument selects the effective repository for both
planning and attribution. Edits through Antigravity's dedicated file tools
never reach the touch pipeline at all (unpinned tool names — the caveat in
`references/install-and-trust.md`), so they surface nothing. The one noisy
case is the advisor's own scoped scan failing to complete (see "The advisor:
what a held command sees" above): that still fails open, but visibly — a
warning, delivered with the invocation's ephemeral report, names the failure and
...
even though nothing was held. Every hook exits 0 — Antigravity gives hooks no
exit-code channel — and hook timeouts are configured in **seconds** under its
`hooks.json` (Claude Code's equivalent is milliseconds); if you're comparing
harnesses' hook definitions, don't read the raw number across without
converting.
