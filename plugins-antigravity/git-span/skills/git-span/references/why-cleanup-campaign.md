...
Dispatch each fork unit by delegating to a subagent with `invoke_subagent`.
Delegated subagents are not documented to inherit your conversation, so the
delegation message must carry everything the subagent needs: point it at this
reference by absolute path (the procedure below lives here) and inline the why
standard's application to its unit:

```text
Read [absolute path to ./why-cleanup-campaign.md] and run its Cleanup
procedure on the unit below. Do not commit.

Bring these <N> spans' whys up to the why standard inlined above (component:
<label> — connected via <shared-file>).
...
(Context: these spans share <shared-file>. Clean sibling spans also anchoring
it: <list>. <Overlap or duplicate-candidate flag if any>.)
```

Check state with `manage_subagents` while units run, and re-engage a live
subagent with `send_message` when a unit needs a correction.
