...
function pendingInjectionsDir(layout, conversationId) {
  return nodePath5.join(layout.dir(conversationId), "antigravity-pending-injections");
}
...
function createHandler(git = createDefaultGitExecutor(), executors = createDefaultAdvisorExecutors(), memoFactory = createDiskAdvisorMemoState, layout = DEFAULT_SESSION_LAYOUT) {
  return async (input, ctx) => {
    try {
      ...
      if (result.kind === "environmental" || result.kind === "scan-failed" || result.kind === "semantic-drift-report" || result.kind === "uncovered-writes-report") {
