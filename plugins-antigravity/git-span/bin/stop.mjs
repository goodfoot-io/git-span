...
function pendingInjectionsDir(layout, conversationId) {
  return nodePath2.join(layout.dir(conversationId), "antigravity-pending-injections");
}
function cleanupCallScopedState(layout, conversationId, now = Date.now()) {
  const targets = [
  ...
  for (const dirPath of targets) {
    try {
      ...
      const trashPath = nodePath2.join(
        layout.trashDir,
        `${nodePath2.basename(dirPath)}${SESSION_TRASH_MARKER}${process.pid}-${now.toString(36)}-${randomBytes(2).toString("hex")}`
      );
...
// packages/agent-hooks/src/antigravity/stop.ts
...
// packages/agent-hooks/src/antigravity/stop-entry.ts
