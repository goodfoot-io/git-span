...
function postInvocationHook(config, handler) {
...
function postInvocationOutput(options = {}) {
...
function drainPendingInjections(layout, conversationId) {
  const dir = pendingInjectionsDir(layout, conversationId);
  let names;
  try {
    names = fs2.readdirSync(dir).filter((name) => name.endsWith(".txt"));
  } catch {
    return [];
  }
  names.sort();
  ...
  for (const name of names) {
    const file = nodePath2.join(dir, name);
    try {
      blocks.push(fs2.readFileSync(file, "utf8"));
      fs2.unlinkSync(file);
    } catch (err) {
    }
  }
