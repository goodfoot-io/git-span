...
function parseCompoundStages(command, split, options, maxCandidates, parse) {
  ...
  for (let index = 0; index < split.stages.length; index += 1) {
    ...
    const join8 = stage.precededBy === "and" ? "&&" : stage.precededBy === "or" ? "||" : void 0;
    resolved.push(
      ...child.resolved.map((match) => ({
        ...match,
        span: { ...match.span, simpleCommandIndex: index, join: join8 }
      }))
    );
...
function applyJoinFilter(order, groups, guardByIndex, computed) {
  ...
  let prevIndex = null;
  for (const idx of order) {
    const join8 = joinOfCommand(idx, groups, guardByIndex);
    const prevVerdict = prevIndex !== null ? effective.get(prevIndex) : void 0;
    if (prevVerdict !== void 0 && join8 !== void 0) {
      if (join8 === "&&" && prevVerdict === "failed" || join8 === "||" && prevVerdict === "succeeded") {
        effective.set(idx, join8 === "&&" ? "failed" : "succeeded");
        skipped.add(idx);
        prevIndex = idx;
        continue;
      }
...
function failureBashResponse(input) {
  ...
  const response = record2.tool_response !== null && typeof record2.tool_response === "object" ? { ...record2.tool_response } : {};
...
function resolveCallCwd(call, workspacePaths) {
...
function toolCallDir(layout, conversationId) {
  return nodePath6.join(layout.dir(conversationId), "antigravity-tool-calls");
}
function pendingInjectionsDir(layout, conversationId) {
  return nodePath6.join(layout.dir(conversationId), "antigravity-pending-injections");
}
...
function takeToolCall(layout, conversationId, stepIdx) {
  const file = nodePath6.join(toolCallDir(layout, conversationId), `${stepIdx}.json`);
  let raw;
  try {
  ...
  try {
    ...
    if (typeof record2.name !== "string") return null;
    const args = record2.args;
    if (args === null || typeof args !== "object" || Array.isArray(args)) return null;
    return { name: record2.name, args };
  } catch {
    return null;
  }
}
var appendSequence = 0;
function appendPendingInjection(layout, conversationId, block) {
  ...
  writeAtomic(nodePath6.join(dir, name), block);
}

// packages/agent-hooks/src/antigravity/post-tool-use.ts
  ...
  return async (input, ctx) => {
    try {
      ...
      const blocks = await runLayeredBashTouches(
        ...
        input.error === void 0 ? void 0 : failureBashResponse({ tool_response: void 0 }),
        executors,
...
disableUpdateCheck();
var post_tool_use_default = postToolUseHook({ matcher: "run_command", timeout: 1e4 }, createHandler());

// packages/agent-hooks/src/antigravity/post-tool-use-entry.ts
