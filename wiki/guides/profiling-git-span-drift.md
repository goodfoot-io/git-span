...
Writes one CSV row per resolved anchor to `/tmp/trace.csv`. The flag ([`src/cli/mod.rs`](../../packages/git-span/src/cli/mod.rs#L309-L315)) is opt-in; without it, no per-anchor instrumentation runs.
