...
Writes one CSV row per resolved anchor to `/tmp/trace.csv`. The flag ([`src/cli/mod.rs`](../../packages/git-span/src/cli/mod.rs#L491-L497)) is opt-in; without it, no per-anchor instrumentation runs.
...
| `fast_path` | bool | `true` if the anchor returned via [`clean_head_fast_path`](../../packages/git-span/src/resolver/engine/anchor.rs#L1929-L1993) |
