...
```xml
<invoke name="Agent">
<parameter name="description" string="true">Reconcile drifted git spans</parameter>
<parameter name="subagent_type" string="true">fork</parameter>
<parameter name="name" string="true">reconcile</parameter>
<parameter name="prompt" string="true">
Read this skill's references/reconcile.md and run its workflow. You are a forked teammate, not the session's main agent: never commit — after validation, report your findings, every anchor diff, and the commit commands for the session's main agent to run.
</parameter>
</invoke>
```
