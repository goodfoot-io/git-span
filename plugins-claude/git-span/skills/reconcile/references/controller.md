...
You are the controller — the last-spawned worker, possibly the only worker. You never commit — only the session's main agent commits. `./worker.md` holds the per-anchor procedure and the git allowlist.
...
Send each earlier-spawned worker a disjoint unit via SendMessage: the spans, drifted anchors (path, range, CHANGED/DELETED), why, shared files, non-drifted context spans, and any range-overlap flags. Keep a unit for yourself. When you are the only worker, keep every unit and execute it yourself.

Follow `./worker.md`'s per-anchor procedure for your own unit.

## Collect and validate

Collect the workers' reports, then run the final validation:
...
## Report to the session's main agent

Report the findings, every anchor diff, the fork count, per-fork timing, the validation status, and the commit commands — `git add <changed-anchor-paths> .span` (omit content paths when no anchor changed) then `git commit -o .span <changed-anchor-paths> -m "Restore spanned relationships"`.
