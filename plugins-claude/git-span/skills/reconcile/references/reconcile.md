...
The agent running this workflow plays the "main agent" role below — the session's main agent running inline, or a forked teammate dispatched while the session's main agent works on other tasks. Only the session's main agent commits: a dispatched teammate stops after validation and reports back so the session's main agent can stage and commit.
...
Only the session's main agent commits.
...
If you are a dispatched teammate, stop here — report the results, every anchor
diff, and the commit commands below for the session's main agent to run.
Otherwise, as the session's main agent:
