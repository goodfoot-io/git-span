...
The block contains the output of `git span list <intersecting-span-names…>` — the human-readable span listing for every span whose anchors on the touched file intersect the tool's line range. If any surfaced span is also stale, the block appends a line per drifted span pointing at `git span history <name>` instead of leaving the drift silent.

## When the block appears

The hook fires on `Read`, `Edit`, and `Write`. A block is emitted when all of the following hold:

- The tool resolves to a single file inside a git repository, and the file is not git-ignored.
...
- **Full-content replacements**: `Write` where every existing line changes (no common prefix or suffix with what's on disk).
- **Whole-file anchors**: anchors in span files that have no `#L…` range are excluded from matching.
- **Gitignored or non-repo files**: the hook skips quietly if the file is git-ignored or not inside a working tree.
