...
These are valid on every subcommand (and on the bare `git span` form):
...
(missing file, missing span name, unmatched literal glob).

Silent exit-0 from `git span stale` (and `list`) means the queried scope is
...
`#L<start>-L<end>` line-range addresses or bare span names.
...
`./inspecting-spans.md` § "Trace blast radius".
...
whitespace is semantically meaningful. Config is part of the span file, so it is
versioned and shared by every consumer of the span exactly like the anchors.
...
leaves git's textual conflict markers in the span file. A conflict-markered span
...
git span merge-driver <O> <A> <B> <L>   # git-invoked accelerator (never run by hand)
```
...
conflict-markered `.span/` files into one clean version: it splits the markers
into ours/theirs, **enforces a clean-source precondition** — every source file an
affected span anchors must itself be conflict-free — reads the now-clean source,
...
declining to re-stage that span, and reporting loudly:
...
of `.span/` conflicts collapse in place and never surface (see
...
non-zero (git's native partial-resolution signal), leaving `git span stale --fix`
...
**Known gap: multi-anchor spans mid-merge, before the merge commit.** The
paragraph above describes the conflict-markered `.span/` file case, which
converges correctly. A narrower, currently-broken case: a span with two or
more anchors on the *same file*, both drifting from the same merge, where the
`.span/` file itself carries **no** conflict markers (neither branch touched
it) — only the source conflicted. Running `git span stale --fix` while
