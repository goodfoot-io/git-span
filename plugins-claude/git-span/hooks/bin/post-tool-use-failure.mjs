...
function createSessionLayout(base) {
  ...
  const plannedTouchFile = (sessionId, toolUseId, suffix) =>
...
var DEFAULT_SESSION_LAYOUT = createSessionLayout(nodePath.join(os.homedir(), '.cache', 'git-span', 'session'));
...
import { execFileSync as execFileSync3, execFileSync as execFileSync4 } from 'node:child_process';
...
var WORD_END = /[\s;&|()<>]/;
var COMMAND_OPENER_WORDS = /* @__PURE__ */ new Set([
  'do',
  'then',
  'else',
  'elif',
...
function emitSourceSpan(results, spec, absolutePath, simpleCommandIndex, join7) {
  ...
  results.push({
    ...
    span:
      range === null
        ? { operation: 'read', absolutePath, simpleCommandIndex, join: join7 }
        : {
            operation: 'read',
    ...
    for (const r of contentRedirects) {
      ...
      if (r.op === '>>' || r.op === '&>>') {
        ...
        results.push({
          ...
          span:
            body.length === 0
              ? { operation: 'truncate', absolutePath, simpleCommandIndex, join: join7 }
              : {
                  operation: 'create-overwrite',
                  absolutePath,
                  simpleCommandIndex,
                  ...
                  ...(singlePlainOverwrite && bodyLiteral
                    ? {
                        written: `${body}
`
                      }
            ...
            span:
              body.length === 0
                ...
                : {
                    ...
                    ...(contentRedirects.length === 0 && bodyLiteral
                      ? {
                          written: `${body}
`
                        }
...
function matchFormatter(argv, dirForResolution, simpleCommandIndex, join7, results) {
  ...
  for (const operand of operands) {
    results.push({
      ...
      span: {
        operation: 'modify',
        absolutePath: resolvePath(dirForResolution, operand),
        simpleCommandIndex,
        join: join7
      }
    });
...
function parseCommandDetailed(command, opts = {}) {
  ...
  const emitCandidate = (c, frame, simpleCommandIndex, join7) => {
    ...
    const resolutionDir =
      c.resolverKind === 'fs'
        ? c.dirOverride === void 0
          ? frame.dir
          : isAbsolute2(c.dirOverride)
            ? c.dirOverride
...
function parsePatternCommand(command) {
  ...
  if (argv[0] === 'sed') {
    ...
    return inplace2 && script2 !== null
      ? { kind: 'sed', script: script2, files: files2, backupSuffix, simpleCommandIndex: 0 }
      : null;
  }
...
var NODE_NAME_ALIAS_PATTERN = new RegExp(`^(?:const|let|var)\\s+(${NODE_NAME_SOURCE})\\s*=\\s*(${NODE_NAME_SOURCE})$`);
...
function emitPythonAnchorSlice(ctx, expression, absolutePath) {
  ...
  if (
    ...
    nodePath4.resolve(ctx.cwd, read.path) !== absolutePath
  ) {
    return rejectPython(
...
function emitPythonStructuredDump(ctx, expression, absolutePath) {
  ...
  if (
    ...
    nodePath4.resolve(ctx.cwd, value.path) !== absolutePath ||
    value.keys.length === 0
  ) {
    return rejectPython(
      'unsupported-dataflow',
...
function parseCommandLayered(command, options = {}) {
  ...
  const patternCommand =
    split.malformed === void 0 && split.stages.length === 1 ? parsePatternCommand(split.stages[0].text) : null;
  ...
  const resolved = detailed.flatMap((match) =>
    match.status === 'resolved' ? [{ status: 'resolved', layer: 'shell', idiom: match.idiom, span: match.span }] : []
  );
  const unresolvedMatches = detailed.flatMap((match) =>
    match.status === 'unresolved'
      ? [unresolved('shell', match.idiom, stableReason(match), match.reason, match.fileArg)]
      : []
  );
...
function validRange(value) {
  ...
  const range = value;
  return (
    Number.isSafeInteger(range.start) && Number.isSafeInteger(range.end) && range.start >= 1 && range.end >= range.start
  );
}
...
function normalizePlannedTouchRecord(record2, budgets) {
  if (
    typeof record2 !== 'object' ||
    record2 === null ||
    record2.version !== 1 ||
    typeof record2.sessionId !== 'string' ||
    record2.sessionId.length === 0 ||
    typeof record2.toolUseId !== 'string' ||
    record2.toolUseId.length === 0 ||
    typeof record2.repoRoot !== 'string' ||
    record2.repoRoot.length === 0 ||
    !Number.isFinite(record2.createdAtMs) ||
    record2.createdAtMs < 0 ||
    !Array.isArray(record2.touches)
  ) {
...
function filterTrackedEligibility(candidates, options) {
  ...
  for (const candidate of candidates) {
    ...
    if (
      relativePath === '' ||
      relativePath === '..' ||
      relativePath.startsWith(`..${nodePath4.sep}`) ||
      nodePath4.isAbsolute(relativePath)
    ) {
...
function decodeStatus(value, label) {
  ...
  const code = enumField(
    object.code,
    ['FRESH', 'RESOLVED_PENDING_COMMIT', 'MOVED', 'CHANGED', 'DELETED', 'CONFLICT', 'SUBMODULE', 'CONTENT_UNAVAILABLE'],
    `${label}.code`
  );
  ...
  const reason = enumField(
    object.reason,
    ['LFS_NOT_FETCHED', 'LFS_NOT_INSTALLED', 'PROMISOR_MISSING', 'SPARSE_EXCLUDED', 'FILTER_FAILED', 'IO_ERROR'],
    `${label}.reason`
  );
...
function selectSurvivors(order, evals, skipped, exitCode) {
  ...
  for (const idx of order) {
    ...
    for (const e of list) {
      ...
      if (e.outcome === 'inconclusive' && e.touch.kind === 'write' && exitCode !== void 0 && exitCode !== 0) continue;
      touches.push(e.touch);
    }
  }
  return touches;
}
async function runBashTouches(
  matches,
  sessionId,
  cwd,
  toolResponse,
...
function parseResponse(input) {
  ...
  const roots =
    magic && worktreeRoot !== null
      ? [worktreeRoot]
      : info.pathArgs.length > 0
        ? info.pathArgs.map((p) => resolvePath2(effectiveDir, p))
        : [effectiveDir];
  const singleFileArg = info.pathArgs.length === 1 ? info.pathArgs[0] : null;
  const oneFileEligible =
    info.numbered && !info.withFilename && singleFileArg !== null && isFile(resolvePath2(effectiveDir, singleFileArg));
...
function planEvidence(matches, requirements) {
  ...
  const ranges = matches.flatMap(({ span }) =>
    span.lineStart === void 0 ? [] : [{ start: span.lineStart, end: span.lineEnd ?? span.lineStart }]
  );
  if (expectedContent !== void 0) {
    return {
      kind: 'content-digest',
  ...
  for (const [key, matches] of groups) {
    ...
    touches.push({
      ...
      ranges: matches.flatMap(({ span: item }) =>
        item.lineStart === void 0 ? [] : [{ start: item.lineStart, end: item.lineEnd ?? item.lineStart }]
      ),
...
function plannedSpans(record2, cwd, logger2) {
  ...
  const cwdInsidePlannedRepo =
    relativeCwd === '' ||
    (relativeCwd !== '..' && !relativeCwd.startsWith(`..${nodePath5.sep}`) && !nodePath5.isAbsolute(relativeCwd));
...
function filterPostTracked(matches, responseSpans, cwd, preTrackedPaths, preTrackedDeletes) {
  const guards = matches.filter((match) => match.status === 'builtin-guard');
...
async function runLayeredBashTouches(
  command,
  cwd,
  sessionId,
  toolUseId,
  ...
  const ordinary = parsed.resolved
  ...
  const joinByIndex = new Map([
    ...parsed.resolved.flatMap(({ span }) => (span.join === void 0 ? [] : [[span.simpleCommandIndex, span.join]])),
    ...detailed.flatMap((match) =>
      match.status === 'resolved' && match.span.join !== void 0
        ? [[match.span.simpleCommandIndex, match.span.join]]
        : []
    )
  ]);
  ...
  const guards = detailed.filter((match) => match.status === 'builtin-guard');
  ...
  const preTrackedDeletes = new Set(
    (record2?.touches ?? [])
      ...
      .map(({ repoRelativePath }) => nodePath5.join(record2.repoRoot, repoRelativePath))
  );
...
function failureBashResponse(input) {
  const record2 = input !== null && typeof input === 'object' ? input : {};
  const response =
    record2.tool_response !== null && typeof record2.tool_response === 'object' ? { ...record2.tool_response } : {};
  if (integerExitStatus(response) === void 0) response.exitStatus = 1;
  if (record2.is_interrupt === true) response.is_interrupt = true;
  return response;
}
...
function createHandler2(
  executors = createDefaultTouchExecutors(),
  memoFactory = createDiskMemoStore,
